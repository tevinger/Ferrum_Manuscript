...
watershed_colors <- c(  
    ...
    "Red Dog" = "black",
    "Su-Lik" = "purple",
    "False Wager Creek" = "red",
    "Drenchwater Creek" = "green"
    #"Springs" = "blue"
    )
...
write_xlsx(Seep_ficklin, "C:/Users/tevinger/Downloads/Seep Ficklin Data.xlsx")
```
...
write_xlsx(impaired_trib_ficklin, "C:/Users/tevinger/Downloads/impaired_trib_ficklin.xlsx")
```
...
```{r}
G_and_K_2009_Ficklin <- read_excel("C:/Users/tevinger/Box/Poulin Lab ETOX/Project Folders/Alaska BITE_HEAT (Taylor Evinger)/Evinger Manuscripts/Ferrum Manuscript/Datasheets/Ferrum Manuscript Datasheets/Graham and Kelley 2009 Ficklin Data.xlsx")

G_and_K_2009_Ficklin_mod <- G_and_K_2009_Ficklin %>%
  filter(Watershed != "Springs")

write_xlsx(G_and_K_2009_Ficklin_mod, "C:/Users/tevinger/Downloads/G_and_K_2009_Ficklin.xlsx")
```
...
Seep_ficklin_plot <- 
  ...
  # Data points
    ...
    geom_point(data = G_and_K_2009_Ficklin, # impaired data
             aes(x = pH, y = Ficklin_metals, 
                 #color = Watershed, 
                 color = "grey",
                 shape = Watershed),
             size = 3) +
  ...
  geom_point(data = impaired_trib_ficklin, # impaired data
             ...
             size = 4, shape = 21, color = "black", stroke = 1.5, alpha = .8) +
  
  geom_point(size = 4.5, shape = 23, color = "black", stroke = 1.5) + #seep data
  ...
  scale_x_continuous(limits = c(2, 8.33),
                     breaks = c(2,2.5,3,3.5,4,4.5,5,5.5,6,6.5,7,7.5,8),
                     labels = c(2, "","","", 4,"", "","", 6,"","", "", 8),
  ...
  theme(
    ...
    legend.spacing.y = unit(0.1, "cm")# center title
  ) +
  guides(fill = "none")

Seep_ficklin_plot

ggsave("C:/Users/tevinger/Box/Poulin Lab ETOX/Project Folders/Alaska BITE_HEAT (Taylor Evinger)/Evinger Manuscripts/Ferrum Manuscript/Figures/Ficklin Diagram/Seeps Ficklin Diagram_v4_with lit values.png", Seep_ficklin_plot, width = 5, height = 5, dpi = 300, bg = "transparent")
...
Seep_ficklin_plot_legend <- get_legend(Seep_ficklin_plot)

library(grid)

grid.draw(Seep_ficklin_plot_legend)

ggsave('C:/Users/tevinger/Downloads/Seep_ficklin_plot_legend.png', 
       Seep_ficklin_plot_legend, width = 5, height = 1, dpi = 250)
```

### pie charts for metal composition
```{r}
Seep_ficklin_percent <- Seep_ficklin %>%
  dplyr::select(-c(3,4,5,9)) %>%
  mutate(total = rowSums(across(6:11))) %>%
  mutate(across(
    6:11,
    ~ .x / total * 100,
    .names = "{.col}_pct"
...
```{r}
Seep_ficklin_percent_long <- Seep_ficklin_percent %>%
  pivot_longer(
    cols = ends_with("_pct"),
    names_to = "Component",
    values_to = "Percent"
  ) %>%
  filter(!Field_label %in% c("ANF_Tseep", "ANF_Tseep_source4", "ANF_Tseep_source5")) %>%
  mutate(
    Component = factor(Component,
                       levels = c("f_Pb_mcg_per_l_pct",
        "f_Cd_mcg_per_l_pct",
        "f_Cu_mcg_per_l_pct",
        "f_Co_mcg_per_l_pct",
        "f_Ni_mcg_per_l_pct",
...
# write_xlsx(Seep_ficklin_percent_long, "C:/Users/tevinger/Downloads/Ficklin Diagram percent composition.xlsx")
```

```{r}
cb_palette_metals_pct <- c(
  #"#D55E00", "#ffce96", "#5d455e", 
  "#003f5c", "#752c48", "#ffca67", "#abb3e2", "#fdb5e1","#606fc7"
  # "#003f5c", "#752c48", "#ffca67", "#80acff", "#fdb5e1","#3a5e9b"
  #"#53B0AE", "#bc5090", "#ff6361", "#01796f",  "#bac4c8",
  #"#bc5259", "#c5e6a1", "#ab694e","#8dbfcb",  "#4eab69", "#6d7a6b"
  
)

metals_ordered_pct <- c(
  "f_Pb_mcg_per_l_pct" = "#752c48",
  "f_Cd_mcg_per_l_pct" = "#003f5c", 
  "f_Cu_mcg_per_l_pct" = "#ffca67", 
  "f_Co_mcg_per_l_pct" = "#fdb5e1", 
  "f_Ni_mcg_per_l_pct" = "#abb3e2", 
  "f_Zn_mcg_per_l_pct" = "#606fc7")

metal_colors_pct <- cb_palette_metals_pct
#names(metal_colors) <- metals_ordered

Seep_ficklin_percent_long <- Seep_ficklin_percent_long %>%
  mutate(
    Field_label = factor(
      Field_label, 
      levels = c("ANF_Tseep_source1", "TukC_seep1", "TukC_seep2", "SAT2.5_seep", "NakC_seep_1")
    )
  )
```

```{r}
stacked_pct_plot <- ggplot(Seep_ficklin_percent_long,
                           aes(x = Field_label,
                               y = Percent,
                               fill = Component)) +
  scale_fill_manual(values= metals_ordered_pct) +
  geom_col(width = 0.4) +
  labs(
    x = NULL,
    y = "Percent of Total (%)",
  ...
  scale_x_discrete(expand = expansion(mult = c(0.08, 0.2))) +
  theme(
    panel.grid.minor = element_blank(),
  panel.grid.major = element_blank(),
  panel.background = element_rect(fill = "white"),
...
# ggsave("C:/Users/tevinger/Box/Poulin Lab ETOX/Project Folders/Alaska BITE_HEAT (Taylor Evinger)/Evinger Manuscripts/Ferrum Manuscript/Figures/Ficklin Diagram/Ficklin Diagram metal composition_FINAL.png", stacked_pct_plot, width = 4, height = 4, dpi = 300)
```

```{r}
pie_plot <- Seep_ficklin_percent_long %>%
  filter(Field_label == "ANF_Tseep") %>%
  ggplot(aes(x = Field_label, y = Percent, fill = Component)) +
  geom_col(position = "fill", width = 1, color = "black") +
  coord_polar(theta = "y") +
  labs(
    x = NULL,
    y = NULL,
...
ggsave("C:/Users/tevinger/Box/Poulin Lab ETOX/Project Folders/Alaska BITE_HEAT (Taylor Evinger)/Evinger Manuscripts/Ferrum Manuscript/Figures/Ficklin Diagram/Ficklin Diagram_everything without upstream.png", Ficklin_plot, width = 5, height = 5, dpi = 300, bg = "transparent")
```
...
Piper_Diagram_meq$SS <- as.numeric(factor(Piper_Diagram_meq$Watershed))

# assign colors to each row based on the watershed
Piper_Diagram_meq$color <- watershed_colors[Piper_Diagram_meq$Watershed]
...
AA.pl <- with(Piper_Diagram_meq, piperPlot(Ca.meq, Mg.meq, Na.meq,
...
# unique_sites <- unique(Piper_Diagram_meq$SS)
...
ion_percentage_in_piper <- Piper_Diagram_meq %>% # this currently doesn't include CO3 but can be added
  mutate(
    #NaK = Na + K,
...
```{r}
# set numeric group ID
Piper_Diagram_meq$SS <- as.numeric(factor(Piper_Diagram_meq$Watershed))

# assign colors by watershed
Piper_Diagram_meq$color <- watershed_colors[Piper_Diagram_meq$Watershed]

AA.pl <- with(Piper_Diagram_meq,
              piperPlot(Ca.meq, Mg.meq, Na.meq,
                        Cl.meq, HCO3.meq, SO4.meq,
                        Plot = list(name = SS, color = color),
                        zCat.title = "Sodium",
                        xAn.title = "Chloride",
                        yAn.title = "Bicarbonate"))
...
# 7. Seep Buffering
```{r}
All_Seep_Data_molar <- read_excel("C:/Users/tevinger/Box/Poulin Lab ETOX/Project Folders/Alaska BITE_HEAT (Taylor Evinger)/Evinger Manuscripts/Ferrum Manuscript/Datasheets/Ferrum Manuscript Datasheets/All_seep_Data_molar.xlsx")
```

```{r}
Seep_buffering_plot <- 
  ggplot(All_Seep_Data_molar, aes(x = f_SO4_mol_L, y = `H+_mol_L`, fill = Watershed)) +
  # Add 1:1 line
    geom_abline(intercept = 0, slope = 1, color = "grey30", linetype = "dashed", linewidth = 0.8) +
    
    # Add 1:2.67 line (slope = 1/2.67 ≈ 0.374)
    geom_abline(intercept = 0, slope = 2.67, color = "red", linetype = "dotdash", linewidth = 0.8) +
  
    geom_point(size = 4, shape = 21, stroke = 1.5, color = "black") +
  scale_fill_manual(values = watershed_colors) +
  scale_x_log10() +
    scale_y_log10() +
  theme_minimal() +
...
```{r}
Seep_buffering_data <- read_excel("C:/Users/tevinger/Box/Poulin Lab ETOX/Project Folders/Alaska BITE_HEAT (Taylor Evinger)/Evinger Manuscripts/Ferrum Manuscript/Datasheets/Ferrum Manuscript Datasheets/Seep Buffering Calculation.xlsx",
                                  sheet = 2)
...
```{r}
Seep_buffering_plot_pH <- 
  ggplot(Seep_buffering_data, aes(fill = Watershed)) +
  # Add 1:1 line
    geom_abline(intercept = 0, slope = 1, color = "grey30", linetype = "dashed", linewidth = 0.8) +
  ...
  #low theoretical pH value
    # geom_point(aes(x = measured_pH, y = low_theoretical_pH), size = 4, shape = 21, stroke = 1.5, color = "black") +
  
    #high theoretical pH value
    geom_point(aes(x = high_theoretical_pH_SO4, y = measured_pH), size = 4, shape = 21, stroke = 1.5, color = "black") +
  
  # Labels
  annotate("text", x = 3,    y = 5.3, label = "Buffered", size = 5, hjust = 0.5) +
  annotate("text", x = 3,    y = 5.05, label = "Measured pH > Theoretical pH", size = 4, hjust = 0.5) +
  
  
  annotate("text", x =4.8,    y = 2.6, label = "Not Buffered",    size = 5, hjust = 0.5) +
  annotate("text", x = 4.8,    y = 2.35, label = "Measured pH < Theoretical pH", size = 4, hjust = 0.5) +
  ...
  scale_y_continuous(breaks = c(0.5,1,1.5,2,2.5,3,3.5,4,4.5,5,5.5,6,6.5,7),
                     labels = c("","","",2,"","","",4,"","","",6,"",""),
                     expand = expansion(mult = 0, add = 0)) +
  labs(
    title = NULL,
...
#ggsave("C:/Users/tevinger/Box/Poulin Lab ETOX/Project Folders/Alaska BITE_HEAT (Taylor Evinger)/Evinger Manuscripts/Ferrum Manuscript/Figures/Seep Subsurface Buffering.png", Seep_buffering_plot_pH, width = 6.5, height = 6.5, dpi = 300, bg = "transparent")
```

```{r}
Seep_buffering_plot_pH_from_Fe <- 
  ggplot(Seep_buffering_data, aes(fill = Watershed)) +
  # Add 1:1 line
    geom_abline(intercept = 0, slope = 1, color = "grey30", linetype = "dashed", linewidth = 0.8) +
  geom_polygon(data = shade_df,
  ...
  #low theoretical pH value
    ...
    #high theoretical pH value
    geom_point(aes(x = high_theoretical_pH_Fe, y = measured_pH), size = 4, shape = 21, stroke = 1.5, color = "black") +
  
  # Labels
  annotate("text", x = 3,    y = 4.8, label = "Buffered", size = 5, hjust = 0.5) +
  annotate("text", x = 3,    y = 4.55, label = "Measured pH > Theoretical pH", size = 4, hjust = 0.5) +
  
  
  annotate("text", x =4.5,    y = 2.6, label = "Not Buffered",    size = 5, hjust = 0.5) +
  annotate("text", x = 4.5,    y = 2.35, label = "Measured pH < Theoretical pH", size = 4, hjust = 0.5) +
...
ggsave("C:/Users/tevinger/Box/Poulin Lab ETOX/Project Folders/Alaska BITE_HEAT (Taylor Evinger)/Evinger Manuscripts/Ferrum Manuscript/Figures/Seep Subsurface Buffering_Fe.png", Seep_buffering_plot_pH_from_Fe, width = 6.5, height = 6.5, dpi = 300, bg = "transparent")
```
...
Ca_SO4_ratio <- working_Ferrum_data_molar %>%
  ...
  ggplot(aes(x = Watershed, y = Ca_SO4_ratio, fill = New_Grouping)) +
  # --- background shading bands (y: 0–1, 1–2, 2–5) ---
 annotate("rect", xmin = -Inf, xmax = Inf, ymin = -Inf, ymax = 0.6,
...
#          fill = "grey85", color = NA, alpha = 1/3) +
  ...
  scale_fill_manual(values = boxplot_colors, name = "New Grouping") +

  # axes/labels
  coord_flip() +  # optional, for vertical samples
   scale_y_continuous(limits = c(0, 5.1), expand = expansion(mult = c(0.01, 0))) +
...
ggsave("C:/Users/tevinger/Box/Poulin Lab ETOX/Project Folders/Alaska BITE_HEAT (Taylor Evinger)/Evinger Manuscripts/Ferrum Manuscript/Figures/Chemical Weathering Analysis/Ca SO4 molar ratio by watershed_small_V2.png", Ca_SO4_ratio, width = 6, height = 2.5, dpi = 300, scale = 1)
```
...
# 9. Particulate and Filtered Percentage
## Dataframe
```{r}
Ferrum_metal_data <- working_Ferrum_data %>%
  filter(!is.na(New_Grouping)) %>%
  #filter(New_Grouping %in% c("Impaired Tributary", "Seep")) %>%
  mutate(u_REE = ifelse(
...
## Calculate the percentage of filtered and particulate metals in each sample
```{r}
Ferrum_metal_data_long <- Ferrum_metal_data %>%
  pivot_longer(
    cols = matches("^(f_|p_|u_)"),  # all fraction columns
    names_to = c("fraction", "metal"),
    names_pattern = "^(f|p|u)_(.*)$",
    values_to = "value"
  ) %>%
  # pivot wider to get separate f, p, u columns again
  pivot_wider(
    names_from = fraction,
    values_from = value
  ) %>%
  ...
  # drop rows with any NA in fraction columns
  filter(if_all(c(Filtered, Particulate, Unfiltered_total), ~ !is.na(.x))) %>%
  
  # calculate percent ratios
  mutate(
    pct_filtered_of_unfiltered = case_when(
      Particulate == 0 ~ 100,
      TRUE   ~ (Filtered / Unfiltered_total) * 100
    ),
    pct_particulate_of_unfiltered = case_when(
      Particulate == 0 ~ 0,
      TRUE   ~ (Particulate / Unfiltered_total) * 100
    )
  )
```
...
```{r}
pct_df_long <- Ferrum_metal_data_long %>%
  # keep key columns and pivot the % columns
  pivot_longer(
    cols = c(pct_filtered_of_unfiltered, pct_particulate_of_unfiltered),
    names_to = "fraction",
    values_to = "pct_of_unfiltered"
  ) %>%
  mutate(
    # nicer labels for plotting
    fraction = dplyr::recode(
      fraction,
      pct_filtered_of_unfiltered   = "Filtered",
      pct_particulate_of_unfiltered = "Particulate"
    )
    # optional: set New_Grouping order
    # New_Grouping = factor(New_Grouping,
    #                       levels = c("Seep", "Impaired Tributary"))
  ) %>%
  # manually change REE 
  mutate(
  pct_of_unfiltered = case_when(
    SamplingEventID %in% c(
      "KR_Trib2_20230608_1540",
      "AMS_Trib2_20230823_1530",
      "SalmonR_Trib6_20230724_1428",
      "AMS_Trib3_20230823_1400",
      "SheepC_1_20230719_0923",
      "SalmonR_MS_above_Trib6_20230724_1439"
    ) &
      metal == "REE" &
      fraction == "Filtered" ~ 100,
    ...
    SamplingEventID %in% c(
      ...
      metal == "REE" &
      fraction == "Particulate" ~ 0,

    TRUE ~ pct_of_unfiltered
  )
)
```

##Calculate summary stats to use for reporting in paper
```{r}
f_and_p_summary_stats <- pct_df_long %>%
dplyr::select(New_Grouping, fraction, pct_of_unfiltered, metal) %>%
  filter(!metal == "REE") %>% # exclude REE because the individual REEs are in the dataset
#filter(Temporal_status %in% c("pre", "post")) %>%   # keep only the two groups
  group_by(New_Grouping, fraction
           #, metal
           ) %>%
  summarise(
    n        = n(),                     # number of values
    mean     = mean(pct_of_unfiltered, na.rm = TRUE),
    median   = median(pct_of_unfiltered, na.rm = TRUE),
    sd       = sd(pct_of_unfiltered, na.rm = TRUE),
    se       = sd(pct_of_unfiltered, na.rm = TRUE) / sqrt(n()),
    min      = min(pct_of_unfiltered, na.rm = TRUE),
    max      = max(pct_of_unfiltered, na.rm = TRUE),
    .groups = "drop"
  )

view(f_and_p_summary_stats)
```

##Plots for all metals and all samples
```{r}
mean_df <- pct_df_long %>%
  group_by(New_Grouping, fraction) %>%
  summarise(mean_pct = mean(pct_of_unfiltered, na.rm = TRUE)) %>%
  ungroup()
...
```{r}
metal_fraction_bp <- ggplot(
  pct_df_long,
  aes(
    x = New_Grouping,
    y = pct_of_unfiltered,
    fill = New_Grouping,          # color by grouping
    pattern = fraction,           # pattern by fraction (filtered / particulate)
    pattern_fill = fraction,
    group = interaction(New_Grouping, fraction)  # needed for dodging
  )
) +
  ...
  # adjust to match your actual fraction labels exactly
  scale_pattern_manual(values = c("Filtered" = "none", "Particulate" = "stripe"), name = "Metal Phase") +
  scale_pattern_fill_manual(values = c("Filtered" = NA, "Particulate" = "grey20")) +
  labs(
    x = NULL,
    #x = "Grouping",
    y = "Metal Phase Percent (% of total)",
    fill = "Grouping",
    pattern = "Fraction"
  ) +
  theme_classic() +
  ...
  axis.ticks.x = element_blank(),
  axis.text.y = element_text(size = 16, color = "black", face = "bold"),
  axis.title.y = element_text(size = 18, color = "black", face = "bold", vjust = 1.25, margin = margin(r = 0.5)),
  legend.position = "none",
  legend.title = element_blank(),
  legend.background = element_rect(fill = NA),
  legend.text = element_text(size = 12, color = "black", face = "bold"),
  ...
  plot.title = element_text(hjust = 0.5, size = 12, color = "black", face = "bold"),
  axis.text.x = element_blank()
  #axis.text.x = element_text(size = 16, color = "black", face = "bold", hjust = 0.5)
  )

metal_fraction_bp

#ggsave("C:/Users/tevinger/Box/Poulin Lab ETOX/Project Folders/Alaska BITE_HEAT (Taylor Evinger)/Evinger Manuscripts/Ferrum Manuscript/Figures/Metal Phase.png", metal_fraction_bp, width = 3, height = 5, dpi = 300)
```

Indivdual Metal distributions Faceted by Sample Type
```{r}
f_vs_p_by_metal_bp_seep <- pct_df_long %>%
  filter(New_Grouping == "Seep") %>%
  filter(!str_detect(metal, "La|Ce|Dy|Nd|Pr|Y")) %>%
  ...
  scale_fill_manual(values = group_colors) +
   scale_pattern_manual(values = c("Filtered" = "none", "Particulate" = "stripe")) +
  scale_pattern_fill_manual(values = c("Filtered" = NA, "Particulate" = "grey20")) +
  facet_grid(~ metal, scales = "free_y") +
  labs(
    x = NULL,
    y = "% of Total Concentration",
    fill = "Grouping"
  ) +
  theme_classic() +
  theme(
    ...
    strip.text.x = element_text(size = 5)  # metals on top
  )

#f_vs_p_by_metal_bp_seep

ggsave("C:/Users/tevinger/Box/Poulin Lab ETOX/Project Folders/Alaska BITE_HEAT (Taylor Evinger)/Evinger Manuscripts/Ferrum Manuscript/Figures/Metal Phase/Seep.png", f_vs_p_by_metal_bp_seep, width = 8, height = 5, dpi = 300)
```

```{r}
f_vs_p_by_metal_bp_impaired_trib <- pct_df_long %>%
  filter(New_Grouping == "Impaired Tributary") %>%
  filter(!str_detect(metal, "La|Ce|Dy|Nd|Pr|Y")) %>%
  ggplot(aes(x = fraction,
           y = pct_of_unfiltered,
...
#f_vs_p_by_metal_bp_impaired_trib

ggsave("C:/Users/tevinger/Box/Poulin Lab ETOX/Project Folders/Alaska BITE_HEAT (Taylor Evinger)/Evinger Manuscripts/Ferrum Manuscript/Figures/Metal Phase/Impaired Trib.png", f_vs_p_by_metal_bp_impaired_trib, width = 8, height = 5, dpi = 300)
```

```{r}
f_vs_p_by_metal_bp_unimpaired_trib <- pct_df_long %>%
...
#f_vs_p_by_metal_bp_unimpaired_trib

ggsave("C:/Users/tevinger/Box/Poulin Lab ETOX/Project Folders/Alaska BITE_HEAT (Taylor Evinger)/Evinger Manuscripts/Ferrum Manuscript/Figures/Metal Phase/Unimpaired Trib.png", f_vs_p_by_metal_bp_unimpaired_trib, width = 12.5, height = 6, dpi = 300)
```

```{r}
f_vs_p_by_metal_bp_upstream <- pct_df_long %>%
  filter(New_Grouping == "Upstream") %>%
  ggplot(aes(x = fraction,
           y = pct_of_unfiltered,
           fill = New_Grouping,
...
#f_vs_p_by_metal_bp_upstream

ggsave("C:/Users/tevinger/Box/Poulin Lab ETOX/Project Folders/Alaska BITE_HEAT (Taylor Evinger)/Evinger Manuscripts/Ferrum Manuscript/Figures/Metal Phase/Upstream.png", f_vs_p_by_metal_bp_upstream, width = 12.5, height = 6, dpi = 300)
```

```{r}
f_vs_p_by_metal_bp_downstream <- pct_df_long %>%
  filter(New_Grouping == "Downstream MS") %>%
  ggplot(aes(x = fraction,
           y = pct_of_unfiltered,
           fill = New_Grouping,
...
#f_vs_p_by_metal_bp_downstream

ggsave("C:/Users/tevinger/Box/Poulin Lab ETOX/Project Folders/Alaska BITE_HEAT (Taylor Evinger)/Evinger Manuscripts/Ferrum Manuscript/Figures/Metal Phase/Downstream.png", f_vs_p_by_metal_bp_downstream, width = 12.5, height = 6, dpi = 300)
```

##Averages for each metal
```{r}
f_and_p_summary_stats_metal_specific <- pct_df_long %>%
dplyr::select(New_Grouping, fraction, pct_of_unfiltered, metal) %>%
#filter(Temporal_status %in% c("pre", "post")) %>%   # keep only the two groups
  group_by(New_Grouping, fraction
           , metal
...
view(f_and_p_summary_stats_metal_specific)
```

##Plots with averages for each metal by sample type
```{r}
mean_df <- f_and_p_summary_stats_metal_specific %>%
  group_by(New_Grouping, fraction) %>%
  summarise(mean_pct = mean(pct_of_unfiltered, na.rm = TRUE)) %>%
  ungroup()
...
```{r}
f_and_p_summary_stats_metal_specific <- f_and_p_summary_stats_metal_specific %>%
  mutate(
    x_group = factor(
      interaction(New_Grouping, fraction, sep = "_"),
      levels = c("Unimpaired Tributary_Filtered", "Unimpaired Tributary_Particulate", "Upstream_Filtered", "Upstream_Particulate", "Seep_Filtered", "Seep_Particulate", "Impaired Tributary_Filtered", "Impaired Tributary_Particulate", "Downstream MS_Filtered", "Downstream MS_Particulate")
    )
  )

pct_df_long <- pct_df_long %>%
  mutate(
    x_group = factor(
      interaction(New_Grouping, fraction, sep = "_"),
      levels = c("Unimpaired Tributary_Filtered", "Unimpaired Tributary_Particulate", "Upstream_Filtered", "Upstream_Particulate", "Seep_Filtered", "Seep_Particulate", "Impaired Tributary_Filtered", "Impaired Tributary_Particulate", "Downstream MS_Filtered", "Downstream MS_Particulate")
...
```{r}
metal_fraction_bp <- f_and_p_summary_stats_metal_specific %>%
  ggplot(
    aes(
    x = x_group,
    y = mean,
  ...
  labs(
    x = NULL,
    #x = "Grouping",
    y = "Average Metal Fraction Percent (% of total metal concentration)",
    fill = "Grouping",
    pattern = "Fraction"
  ) +
  theme_classic() +
  ...
  axis.ticks.x = element_blank(),
  axis.text.y = element_text(size = 16, color = "black", face = "bold"),
  axis.title.y = element_text(size = 12, color = "black", face = "bold", vjust = 1.25, margin = margin(r = 0.5)),
  legend.position = "none",
  legend.title = element_blank(),
  legend.background = element_rect(fill = NA),
  legend.text = element_text(size = 12, color = "black", face = "bold"),
...
ggsave("C:/Users/tevinger/Box/Poulin Lab ETOX/Project Folders/Alaska BITE_HEAT (Taylor Evinger)/Evinger Manuscripts/Ferrum Manuscript/Figures/Metal Phase/Site classification group_means of fraction.png", metal_fraction_bp, width = 6.5, height = 6, dpi = 300)
```

##Stacked Barplots
```{r}
bar_df <- pct_df_long %>% 
  group_by(New_Grouping, metal, fraction) %>% 
  summarise(mean_pct = mean(pct_of_unfiltered, na.rm = TRUE),
            .groups = "drop")
```
...
stacked_bar <- ggplot(
  ...
  facet_wrap(~ metal, nrow = 3) +   # tweak rows/cols as you like
  coord_cartesian(ylim = c(0, 100)) +
  labs(
    x   = NULL,
    y   = "Mean % of unfiltered total",
    fill = "Metal phase"
  ) +
  theme_classic() +
  theme(
...
stacked_bar <- ggplot(
  ...
  geom_col(position = "stack", colour = "black") +
  facet_wrap(~ New_Grouping, nrow = 3) +   # tweak rows/cols as you like
  coord_cartesian(ylim = c(0, 100)) +
  labs(
    x   = NULL,
    y   = "Mean % of unfiltered total",
...
# Frey et al., 2007 Data
```{r}
# Load Data
Frey_et_al_2007_df <- read_excel("C:/Users/tevinger/Downloads/combined_images.xlsx", 
                                      sheet = 2, 
                                      col_names = FALSE)
view(Frey_et_al_2007_df)
```

```{r}
# 1. Flatten the single-row df to a vector
vals <- unlist(Frey_et_al_2007_df, use.names = FALSE)

# 2. Find positions where entries start with "KF"
kf_pos <- grep("^KF", vals)

# 3. Build chunks: each chunk = KF_i ... just before KF_{i+1}
chunks <- lapply(seq_along(kf_pos), function(i) {
  start <- kf_pos[i]
  end   <- if (i < length(kf_pos)) kf_pos[i + 1] - 1 else length(vals)
  vals[start:end]
})

# 4. Find the maximum chunk width
max_len <- max(lengths(chunks))
min_len <- min(lengths(chunks))


# 4. Turn chunks into individual 1-row data frames,
#    padding those with only 20 entries by inserting NA after the KF cell
dfs <- lapply(chunks, function(x) {
  
  len <- length(x)
  
  if (len == 20) {
    # insert TWO NA columns after KF
    x <- append(x, c(NA, NA), after = 1)
    
  } else if (len == 21) {
    # insert ONE NA column after KF
    x <- append(x, NA, after = 1)
    
  } else if (len == 22) {
    # do nothing, correct width
    x <- x
    
  } else {
    warning("Chunk starting with ", x[1], " has unexpected length: ", len)
  }
  
  as.data.frame(t(x), stringsAsFactors = FALSE)
})

# 5. Give them all consistent column names
n_cols <- 22
colnames_all <- paste0("V", seq_len(n_cols))
dfs <- lapply(dfs, function(d) {
  colnames(d) <- colnames_all
  d
})

# 6. Bind back together into one data frame
Frey_et_al_2007_Table_1 <- do.call(rbind, dfs)
...
colnames(Frey_et_al_2007_Table_1) <- new_colnames
```

```{r}
write_xlsx(Frey_et_al_2007_Table_1,
           "C:/Users/tevinger/Downloads/Frey_et_al_2007_Table_1.xlsx")
```

# pH conversions
```{r}
convert_pH_to_H <- function(pH) {
  10^(-pH)
}
```

```{r}
convert_pH_to_H(8.33)
convert_pH_to_H(8.4)
convert_pH_to_H(8.07)
convert_pH_to_H(7.74)
```
