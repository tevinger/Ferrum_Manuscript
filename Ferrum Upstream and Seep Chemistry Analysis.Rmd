...
working_Ferrum_data <- read_excel("C:/Users/tevinger/OneDrive - University of California, Davis/Lab/Alaska Projects/Final Ferrum Manuscript Data Sheet_V2.xlsx")
...
working_Ferrum_data <- working_Ferrum_data %>%
  dplyr::select(ParkID,
                ...
                Latitude,
                Longitude,
                sample_collection_date_mm_dd_yy,
                sample_time_h_mm,
                SamplingEventID,
                VisualDescription,
                Site_Classification,
...
#2. Data Analysis
##2.1  Individual data frames
```{r}
Upstream_df <- Upstream_seep_df %>%
  filter(Site_Group == "Upstream")

seep_df <- Upstream_seep_df %>%
  filter(Site_Group == "Seep")

Seep_control_df <- Upstream_seep_df %>%
  filter(Site_Group == "Seep_control")
```
...
##2.4 Upstream Chemistry Barplots
###Plot theme
```{r}
barplot_theme <- theme(
...
Upstream_long <- Upstream_df %>%
  ...
  mutate(Watershed_area = case_when(
    ...
    Watershed == "Tukpahlearik"   ~ 83.24,
    Watershed == "Salmon River"         ~ 1740.33,
    Watershed == "Nakolikurok"    ~ 349.5,
    TRUE ~ NA_real_
  )) %>% 
  mutate(
...
Upstream_f_metals_long <- Upstream_df %>%
  dplyr::select(2,18:103) %>% #select Watershed column and the metal data columns
  pivot_longer(
    cols = 2:87,
    names_to = "Variable",
    values_to = "Value"
...
Seep_df_long <- Seep_df %>%
  dplyr::select(-c(3,4,7,8,9,10,12,13,14,18)) %>% # remove site information columns that aren't necessary
  pivot_longer(
    cols = 6:66,
    names_to = "Variable",
    values_to = "Conc"
  ...
  # Add a column for grouping the metals 
  mutate(Metal_Group = case_when(
    grepl("Al_|Fe_|Mn_|Ba_", Variable) ~ "Major",
    grepl("Pb_|As_|Cd_|Cr_|Co_|Cu_|Zn_|Ni_|U_|Se_", Variable) ~ "Trace",
    grepl("Ce_|Dy_|Nd_|Pr_|La_|Y_", Variable) ~ "REE",
    grepl("K_|Na_", Variable) ~ "Minor Cations",
    grepl("Cl_|NO3_|PO4_", Variable) ~ "Minor Anions",
    grepl("Ca_|Mg_", Variable) ~ "Major Cations",
    grepl("SO4_", Variable) ~ "Sulfate",
    TRUE ~ NA_character_  # Optional: NA if none match
  )) %>%
...
Seep_f_metals_long_summary <- Seep_df_long %>%
  group_by(Variable, Watershed, Metal_Group, Metal_Type) %>%
  summarise(
    mean_value = mean(Conc, na.rm = TRUE),
    sd_value = sd(Conc, na.rm = TRUE),
    Min = if (all(is.na(Conc))) NA_real_ else round(min(Conc, na.rm = TRUE), 5),
    Max = if (all(is.na(Conc))) NA_real_ else round(max(Conc, na.rm = TRUE), 5),     
    Count = n(),
    .groups = "drop"
  )

view(Seep_f_metals_long_summary)
```

##Plot colors and metal order
```{r}
watershed_colors <- c(  
...
df_major_seep <- Seep_f_metals_long_summary %>%
  filter(Metal_Group == "Major") %>%
...
df_trace_seep <- Seep_f_metals_long_summary %>%
  filter(Metal_Group == "Trace") %>%
  mutate(Variable = factor(Variable, levels = trace_order))

#view(df_trace_seep)

df_ree_seep <- Seep_f_metals_long_summary %>%
  filter(Metal_Group == "REE") %>%
  mutate(Variable = factor(Variable, levels = ree_order))

df_major_cations_seep <- Seep_f_metals_long_summary %>%
  filter(Metal_Group == "Major Cations") %>%
  mutate(Variable = factor(Variable, levels = major_cations_order))

df_minor_cations_seep <- Seep_f_metals_long_summary %>%
  filter(Metal_Group == "Minor Cations") %>%
  mutate(Variable = factor(Variable, levels = minor_cations_order))

df_minor_anions_seep <- Seep_f_metals_long_summary %>%
  filter(Metal_Group == "Minor Anions") %>%
  mutate(Variable = factor(Variable, levels = minor_anions_order))

df_sulfate_seep <- Seep_f_metals_long_summary %>%
  filter(Metal_Group == "Sulfate")
```
