...
ggsave("C:/Users/tevinger/Box/Poulin Lab ETOX/Project Folders/Alaska BITE_HEAT (Taylor Evinger)/Evinger Manuscripts/Ferrum Manuscript/Figures/Ficklin Diagram/Seeps Ficklin Diagram_v3_with lit values.png", Seep_ficklin_plot, width = 5, height = 5, dpi = 300, bg = "transparent")
...
Seep_ficklin_percent <- Seep_ficklin %>%
  pivot_longer(
    cols = matches("Co|Zn|Ni|Cu|Cd|Pb"),   # all metal columns
    names_to = "metal",        # name of new column with metal names
    values_to = "concentration"   # name of new column with values
  ) %>%
  mutate


Seep_ficklin_pies <- ggplot(sub, aes(y=sdw.perc, x=2, fill=substrate, color=substrate)) +
...
pct_df_long <- Ferrum_metal_data_long %>%
  ...
  # manually change REE for KR_Trib2 
  mutate(
    pct_of_unfiltered = case_when(
      SamplingEventID == "KR_Trib2_20230608_1540" &
        metal == "REE" &
...
```{r}
f_vs_p_by_metal_bp <- pct_df_long %>%
  filter(New_Grouping == "Seep") %>%
  ...
  labs(
    x = "Grouping",
    y = "% of unfiltered total",
    fill = "Grouping"
  ) +
  theme_classic() +
  theme(
    axis.text.x = element_text(angle = 30, hjust = 1),
    strip.text.x = element_text(size = 7)  # metals on top
  )
...
ggsave("C:/Users/tevinger/Box/Poulin Lab ETOX/Project Folders/Alaska BITE_HEAT (Taylor Evinger)/Evinger Manuscripts/Ferrum Manuscript/Figures/Metal Phase by average per metal.png", metal_fraction_bp, width = 3, height = 5, dpi = 300)
```
