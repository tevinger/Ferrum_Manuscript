...
working_Ferrum_data <- working_Ferrum_data %>%
  mutate(
    ...
    HCO3_meq_L = f_HCO3_mg_L / 61.02,  # HCO₃⁻: 1 eq/mol, 61.02 g/mol
...
```{r}
Seep_metals_plot <- 
  ggplot(All_Seep_Data, aes(x = pH, y = all_sum_of_metals_mg_L, color = Watershed)) +
...
Seep_ficklin_plot <- 
  ggplot(All_Seep_Data, aes(x = pH, y = Ficklin_sum_of_metals_ug_L, color = Watershed)) +
  ...
  geom_point(size = 3, alpha = 3) +           # scatter points
  ...
  # Labels
  #annotate("text", x = 6.25,    y = 85000, label = "Near-neutral,", size = 3.5, hjust = 0.5) +
  #annotate("text", x = 6.25,    y = 65000, label = "Low-metal",    size = 3.5, hjust = 0.5) +
  #annotate("text", x = 4.75, y = 35000, label = "Acid,",        size = 3.5, hjust = 0.5) +
  #annotate("text", x = 4.75, y = 15000, label = "Low-metal",    size = 3.5, hjust = 0.5) +
  theme_minimal() +
  DA_theme +
  theme(
    legend.position = "none",
...
ggsave("C:/Users/tevinger/Box/Poulin Lab ETOX/Project Folders/Alaska BITE_HEAT (Taylor Evinger)/Evinger Manuscripts/Ferrum Manuscript/Figures/Seeps Ficklin Diagram.png", Seep_ficklin_plot, width = 5, height = 5, dpi = 300, bg = "transparent")
...
setPNG("piperplot_practice.png", 8,8)

pipr <- with(meq_L_df, piperPlot(Ca, Mg, Na, Cl, HCO3, SO4, Plot = list(name= rownames(meq_L_df), color = setColor(New_Grouping))))

graphics.off()
```

```{r}
meq_L_df$SS <- meq_L_df$New_Grouping
...
AA.pl <- with(meq_L_df, piperPlot(Ca, Mg, Na, Cl, HCO3, SO4,
  Plot=list(name=SS),
  zCat.title = "Sodium",
  xAn.title = "Chloride",
