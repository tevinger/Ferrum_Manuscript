...
plot_data <- data %>%
  ...
  mutate(Scaled_Mean = case_when(
    Variable %in% c("SO4") ~ mean_value,
    Variable %in% c("Ca", "Mg") ~ mean_value * scaling_factor,
    TRUE ~ NA_real_
  )) %>%
  mutate(Scaled_SD = case_when(
    Variable %in% c("SO4") ~ sd_value,
    Variable %in% c("Ca", "Mg") ~ sd_value * scaling_factor,
    TRUE ~ NA_real_
  ))
```
...
anions_cations_seep <- plot_data %>%
  ...
  scale_y_continuous(
    name = "Sulfate (mg/L)",
    sec.axis = sec_axis(~ . / 10, name = "Calcium and Magnesium (mg/L)") #10 is the scaling factor applied to the calcium and magnesium data
  ) +
  #scale_fill_manual(values = c("Sulfate" = "gray50", "Calcium" = "steelblue", "Magnesium" = "darkgreen")) +
