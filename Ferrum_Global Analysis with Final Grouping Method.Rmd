...
Fe_plot <- ggplot(Fe_long, aes(x = x_group, y = Metal_conc_plot)) +
  ...
  scale_y_log10(
    limits = c(0.00001, 10000000),
    labels = scales::comma,
    breaks = c(0.00001, 0.01, 0.1, 1, 10, 100, 10000, 10000000),
    minor_breaks = NULL
  ) +
  annotation_logticks(sides = "l") +
  labs(
...
Al_long <- Al_df %>%
  ...
  filter(!is.na(Metal_conc)) %>% # filtering out here means only the f or p value that is NA is excluded, not both measurements for the same sample
  mutate(Metal_conc_plot = Metal_conc + 0.1)
...
scale_y_log10(
    ...
    labels = scales::comma,
    breaks = c(0.01, 1, 10, 100, 1000, 10000, 100000, 1000000, 10000000),
    minor_breaks = NULL
  ) +
  annotation_logticks(sides = "l") +
  labs(
...
```{r}
# Select Ni data
Ni_df <- Alaska_DataRelease_2022_2023_Ferrum_V8 %>%
  dplyr::select(SamplingEventID, Watershed, New_Groups, New_Grouping, 
...
Ni_long <- Ni_df %>%
  ...
  filter(!is.na(Metal_conc)) %>% # filtering out here means only the f or p value that is NA is excluded, not both measurements for the same sample
  mutate(Metal_conc_plot = Metal_conc + 0.01)
...
```{r}
# Select Cd data
Cd_df <- Alaska_DataRelease_2022_2023_Ferrum_V8 %>%
  dplyr::select(SamplingEventID, Watershed, New_Groups, New_Grouping, 
