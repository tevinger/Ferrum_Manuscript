...
```{r}
# Select Fe data
Fe_df <- Alaska_DataRelease_2022_2023_Ferrum_V8 %>%
  ...
  filter(if_all(c(New_Grouping, f_Fe_mcg_per_l), ~ !is.na(.)))
...
### Fe Violin
```{r}
Fe_plot_violin <- ggplot(Fe_long, aes(x = x_group, y = Metal_conc)) +
  geom_violin(
    aes(fill = New_Groups),
    scale = "width",
    trim = FALSE,
  ...
  stat_summary(
    ...
    geom = "crossbar", width = 0.6, color = "black", size = 0.5
  ) +
  scale_fill_manual(values = boxplot_colors) +
  scale_pattern_manual(
    values = c("f" = "none", "p" = "stripe"),
    guide = "none"
  ) +
  scale_y_log10(
    limits = c(0.1, 1000000),
...
```{r}
# Select Al data
Al_df <- working_Ferrum_data %>%
  dplyr::select(SamplingEventID, Watershed, New_Groups, New_Grouping, f_Al_mcg_per_l, p_Al_mcg_per_l)
...
```{r}
Al_plot <- ggplot(Al_long, aes(x = x_group, y = Metal_conc)) +
  geom_boxplot_pattern(
    aes(fill = New_Groups, pattern = Metal_type, pattern_fill = Metal_type),
    outlier.shape = 16,
    outlier.color = "black",
...
Cd_df <- working_Ferrum_data %>%
  dplyr::select(SamplingEventID, Watershed, New_Groups, New_Grouping, f_Cd_mcg_per_l, p_Cd_mcg_per_l)
...
```{r}
Cd_plot <- ggplot(Cd_long, aes(x = x_group, y = Metal_conc)) +
  geom_boxplot_pattern(
    aes(fill = New_Groups, pattern = Metal_type, pattern_fill = Metal_type),
    outlier.shape = 16,
    outlier.color = "black",
...
```{r}
# Select Co data
Co_df <- working_Ferrum_data %>%
  dplyr::select(SamplingEventID, Watershed, New_Groups, New_Grouping, f_Co_mcg_per_l, p_Co_mcg_per_l)
...
```{r}
Co_plot <- ggplot(Co_long, aes(x = x_group, y = Metal_conc)) +
  geom_boxplot_pattern(
    aes(fill = New_Groups, pattern = Metal_type, pattern_fill = Metal_type),
    outlier.shape = 16,
    outlier.color = "black",
