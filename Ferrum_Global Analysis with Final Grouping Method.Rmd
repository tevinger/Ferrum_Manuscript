...
```{r}
# working_Alaska_DataRelease_2022_2023_Ferrum <- read_excel("C:/Users/tayta/OneDrive/Desktop/Lab/Alaska Projects/Master Spreadsheet/working_Alaska_DataRelease_2022_2023_Ferrum.xlsx", sheet = "Sheet1")

# This is the spreadsheet that has the current metal data in it that I calculated in excel
working_Alaska_DataRelease_2022_2023_Ferrum <- read_excel("C:/Users/tevinger/OneDrive - University of California, Davis/Lab/Alaska Projects/working Ferrum data with p.metals_highlighted_V2_with data analysis.xlsx",
                                                          sheet = "2022_2023 Data")

working_Alaska_DataRelease_2022_2023_Ferrum <- working_Alaska_DataRelease_2022_2023_Ferrum %>% filter(!is.na(Watershed))

unique(working_Alaska_DataRelease_2022_2023_Ferrum$Watershed)

#view(working_Alaska_DataRelease_2022_2023_Ferrum)
```

```{r}
# add distances downstream and elevation data
site_elevation_and_distances <- read_excel("C:/Users/tevinger/OneDrive - University of California, Davis/Lab/Alaska Projects/Master Spreadsheet/site_elevation_and_distances_V2.xlsx", sheet = "USE")

# make everything in this df numeric
site_elevation_and_distances$Distance_km <- as.numeric(site_elevation_and_distances$Distance_km)
site_elevation_and_distances$Elevation_ft <- as.numeric(site_elevation_and_distances$Elevation_ft)
site_elevation_and_distances$Watershed_Area <- as.numeric(site_elevation_and_distances$Watershed_Area)
site_elevation_and_distances$Relative_MS_Watershed_Area <- as.numeric(site_elevation_and_distances$Relative_MS_Watershed_Area)

#view(site_elevation_and_distances)

#site_elevation_and_distances_to_join <- site_elevation_and_distances %>%
  #dplyr::select(SamplingEventID, Distance_km, Elevation_ft)
```

```{r}
# bring in excel file with the site groups
# add Site_Grouping, visual description, site_type
proximity_groups <- read_excel("C:/Users/tevinger/OneDrive - University of California, Davis/Lab/Alaska Projects/Master Spreadsheet/Site_info_to_bring_in.xlsx", sheet = "USE")

#view(proximity_groups)
```

```{r}
# join the site distances, elevation, and groups with the data

# Make sure SamplingEventID is character in all dataframes
working_Alaska_DataRelease_2022_2023_Ferrum <- working_Alaska_DataRelease_2022_2023_Ferrum %>%
  mutate(SamplingEventID = as.character(SamplingEventID))

site_elevation_and_distances <- site_elevation_and_distances %>%
  mutate(SamplingEventID = as.character(SamplingEventID))
...
#join dataframes
working_Alaska_DataRelease_2022_2023_Ferrum_joined <- site_elevation_and_distances %>% 
  left_join(working_Alaska_DataRelease_2022_2023_Ferrum, by = "SamplingEventID") %>%
  left_join(proximity_groups, by = "SamplingEventID") %>%
  relocate(Prox_Groups, RelAcc_Groups, New_Groups, Distance_km, Elevation_ft, VisualDescription,
                Site_Classification,
                Hyd_Site_Classification,
                Hyd_Classification, .after = SamplingEventID)

view(working_Alaska_DataRelease_2022_2023_Ferrum_joined)
```

### Select the 6 watersheds for this study

```{r}
#unique(working_Alaska_DataRelease_2022_2023_Ferrum_joined$Watershed)

# remove watersheds that aren't going to be used in the analysis
selected_Watersheds_2022_2023_Ferrum <- working_Alaska_DataRelease_2022_2023_Ferrum_joined %>% 
  filter(!Subcatchment %in% c("Akillik River", "Noatak River", "Igning River", "Agashashok South Fork"))

# View a list of the watersheds in this df to make sure they're correct
#view(selected_Watersheds_2022_2023_Ferrum)
#str(selected_Watersheds_2022_2023_Ferrum)
unique(selected_Watersheds_2022_2023_Ferrum$Watershed)

# change the watershed names
selected_Watersheds_2022_2023_Ferrum <- selected_Watersheds_2022_2023_Ferrum %>%
  mutate(Watershed = as.character(Watershed)) %>%
  mutate(Watershed = case_when(
    Watershed == "Salmon River Watershed" ~ "Anaktok River",
    Watershed == "Nakolikurok" ~ "Nakolikurok River",
    Watershed == "Tukpahlearik" ~ "Tukpahlearik Creek",
    TRUE ~ Watershed  # keep all other values unchanged
  ))
...
```{r}
#for now keeping site information columns, field measurements, major cations, anions, and DOC
#1:16,18,20,22,24,26,37,38,64,97,99,101,105,107,109,111
# metals are currently in 115:143 and meq/l for cations and anions are in 144:149

Ferrum_2022_2023_data <- Alaska_DataRelease_2022_2023_Ferrum_V8 %>%
  dplyr::select(ParkID,
                Watershed,
                Subcatchment,
                New_Groups,
...
Ferrum_2022_2023_data$Latitude <- as.numeric(Ferrum_2022_2023_data$Latitude)
Ferrum_2022_2023_data$Longitude <- as.numeric(Ferrum_2022_2023_data$Longitude)
Ferrum_2022_2023_data$DOC_mgC_per_l <- as.numeric(Ferrum_2022_2023_data$DOC_mgC_per_l)
#Ferrum_2022_2023_data$sample_collection_date_mm_dd_yy <- as.Date(Ferrum_2022_2023_data$sample_collection_date_mm_dd_yy)

Ferrum_2022_2023_data <- Ferrum_2022_2023_data %>%
  mutate(
    Prox_Grouping = case_when(
    Prox_Groups == 1 ~ "Upstream",
    Prox_Groups == 2 ~ "Seep",
    Prox_Groups == 3 ~ "Impaired Tributary",
    Prox_Groups == 4 ~ "Unimpaired Tributary",
    Prox_Groups == 5 ~ "Downstream MS 1",
    Prox_Groups == 6 ~ "Downstream MS 2",
    TRUE ~ NA_character_
    )
  ) %>%
  mutate(
    RelAcc_Grouping = case_when(
    RelAcc_Groups == 1 ~ "Upstream",
    RelAcc_Groups == 2 ~ "Seep",
    RelAcc_Groups == 3 ~ "Impaired Tributary",
    RelAcc_Groups == 4 ~ "Unimpaired Tributary",
    RelAcc_Groups == 5 ~ "Downstream MS 1",
    RelAcc_Groups == 6 ~ "Downstream MS 2",
    TRUE ~ NA_character_
    )
  ) %>%
  mutate(    
  ...
  relocate(Prox_Grouping, RelAcc_Grouping, New_Grouping, .after = New_Groups)

#str(Ferrum_2022_2023_data)
view(Ferrum_2022_2023_data)
```
...
```{r}
working_Ferrum_data <- Ferrum_2022_2023_data
```

### Metal data
``` {r}
# Code for when I have 2022 data
filtered_metal_df_2022 <- practice_metal_df %>%
  mutate(Extracted_ID = str_extract(SamplingEventID, "\\d{8}_\\d{4}$")) %>%
  mutate(Year = substr(Extracted_ID, 1, 4)) %>%
  filter(Year == "2022") %>%
  dplyr::select(-Extracted_ID, -Year)

view(filtered_metal_df_2022)
```

### Join metal data into df
this won't need to happen once I bring the metal data into the master spreadsheet
```{r}
# This is not using current metal data, but is being used to compare to what I showed in the project meeting 
# The most current metal data needs to be used after all sample data has been acquired (note left on 4/24/25)

FAM_UAM_dL_corrected_df <- read_excel("C:/Users/tayta/OneDrive/Desktop/Lab/Alaska Projects/Master Spreadsheet/Final Metal Data/FAM_UAM_dL_corrected_data_20250314.xlsx")

# join metal data with the rest of the data
new_working_Ferrum_data <- left_join(working_Ferrum_data, FAM_UAM_dL_corrected_df, by = "SamplingEventID")

#view(new_working_Ferrum_data)

#save this data frame to calculate particulate metal concentrations outside of R
write_xlsx(new_working_Ferrum_data, "C:/Users/tayta/Downloads/new working Ferrum data.xlsx")
```


## Bring the modified df back into R - OLD
```{r}
# I calculated the particulate metal concentrations in excel to make sure it was correct and detection limit flags were correctly assigned

# bring that dataframe back into R
working_metal_data <- read_excel("C:/Users/tayta/OneDrive/Desktop/Lab/Alaska Projects/working Ferrum data with p.metals_highlighted_V2_with data analysis.xlsx")

# join new groups with the data
proximity_groups_to_join <- proximity_groups %>%
  dplyr::select(SamplingEventID, New_Groups)

new_working_Ferrum_data <- working_Ferrum_data %>%
left_join(proximity_groups_to_join, by = "SamplingEventID") %>% relocate(New_Groups, .after = SamplingEventID) %>%
  dplyr::select(-Site_Grouping, -Site_Group)

# make things numeric that should be
new_working_Ferrum_data$New_Groups <- as.numeric(new_working_Ferrum_data$New_Groups)
new_working_Ferrum_data$Latitude <- as.numeric(new_working_Ferrum_data$Latitude)
new_working_Ferrum_data$Longitude <- as.numeric(new_working_Ferrum_data$Longitude)
new_working_Ferrum_data$DOC_mgC_per_l <- as.numeric(new_working_Ferrum_data$DOC_mgC_per_l)
#Ferrum_2022_2023_data$sample_collection_date_mm_dd_yy <- as.Date(Ferrum_2022_2023_data$sample_collection_date_mm_dd_yy)

new_working_Ferrum_data <- new_working_Ferrum_data %>%
  mutate(    
    Site_Grouping = case_when(
    New_Groups == 1 ~ "Upstream",
    New_Groups == 2 ~ "Seep",
    New_Groups == 3 ~ "Impaired Tributary",
    New_Groups == 4 ~ "Unimpaired Tributary",
    New_Groups == 5 ~ "Downstream MS 1",
    New_Groups == 6 ~ "Downstream MS 2"
  )) %>%
  relocate(Site_Grouping, Distance_km, Elevation_ft, .after = New_Groups)
...
proximity_plot_legend <- get_legend(pH_plot)

library(grid)

grid.draw(proximity_plot_legend)

ggsave('C:/Users/tayta/Downloads/proximity_plot_legend.png', 
       proximity_plot_legend, width = 5, height = 8, bg = "transparent", dpi = 300)
```
...
```{r}
f_SO4_mg_per_l_plot <- ggplot() + 

  # Boxplot and jitter for New_df_4
  geom_boxplot(data = SO4_New_df_4, aes(x = factor(New_Grouping), y = f_SO4_mg_per_l, fill = factor(New_Groups)),
               width = 0.75, outlier.shape = NA, alpha = 0.5) +
  geom_jitter(data = SO4_New_df_4, aes(x = factor(New_Grouping), y = f_SO4_mg_per_l, fill = Watershed), 
              shape = 21, color = "black", width = 0.15, alpha = 1, size = 2) +

  # Boxplot and jitter for New_df_1
  geom_boxplot(data = SO4_New_df_1, aes(x = factor(New_Grouping), y = f_SO4_mg_per_l, fill = factor(New_Groups)),
               width = 0.75, outlier.shape = NA, alpha = 0.5) +
  geom_jitter(data = SO4_New_df_1, aes(x = factor(New_Grouping), y = f_SO4_mg_per_l, fill = Watershed), 
              shape = 21, color = "black", width = 0.15, alpha = 1, size = 2) +

  # Boxplot and jitter for New_seep_df
  geom_boxplot(data = SO4_New_seep_df, aes(x = factor(New_Grouping), y = f_SO4_mg_per_l, fill = factor(New_Groups)), width = 0.75, outlier.shape = NA, alpha = 0.5) +
  geom_jitter(data = SO4_New_seep_df, aes(x = factor(New_Grouping), y = f_SO4_mg_per_l, fill = Watershed), 
              shape = 21, color = "black", width = 0.15, alpha = 1, size = 2) +

  # Boxplot and jitter for New_df_3
  geom_boxplot(data = SO4_New_df_3, aes(x = factor(New_Grouping), y = f_SO4_mg_per_l, fill = factor(New_Groups)),
               width = 0.75, outlier.shape = NA, alpha = 0.5) +
  geom_jitter(data = SO4_New_df_3, aes(x = factor(New_Grouping), y = f_SO4_mg_per_l, fill = Watershed), 
              shape = 21, color = "black", width = 0.15, alpha = 1, size = 2) +

  # Boxplot and jitter for New_df_6
  geom_boxplot(data = SO4_New_df_6, aes(x = factor(New_Grouping), y = f_SO4_mg_per_l, fill = factor(New_Groups)),
               width = 0.75, outlier.shape = NA, alpha = 0.5) +
  geom_jitter(data = SO4_New_df_6, aes(x = factor(New_Grouping), y = f_SO4_mg_per_l, fill = Watershed), 
              shape = 21, color = "black", width = 0.15, alpha = 1, size = 2) +
...
Fe_df <- working_Ferrum_data %>%
  dplyr::select(SamplingEventID, Watershed, Prox_Groups, RelAcc_Groups, New_Groups, Prox_Grouping, RelAcc_Grouping, New_Grouping, f_Fe_mcg_per_l, p_Fe_mcg_per_l)
...
Cu_df <- working_Ferrum_data %>%
  dplyr::select(SamplingEventID, Watershed, Prox_Groups, RelAcc_Groups, New_Groups, Prox_Grouping, RelAcc_Grouping, New_Grouping, f_Cu_mcg_per_l, p_Cu_mcg_per_l)
...
Cu_plot <- ggplot(Cu_long, aes(x = x_group, y = Metal_conc)) +
  ...
  scale_fill_manual(values = boxplot_colors) +
  scale_pattern_manual(values = c("f" = "none", "p" = "stripe"), name = "Metal Type") +
  scale_pattern_fill_manual(values = c("f" = NA, "p" = "white")) +
  #scale_shape_manual(values = c("f" = 21, "p" = 21)) +
  scale_y_log10(
    limits = c(0.1, 100000),
    labels = scales::comma,
