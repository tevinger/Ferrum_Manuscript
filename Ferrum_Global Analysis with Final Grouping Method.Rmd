...
```{r}
Final_groups_df <- Alaska_DataRelease_2022_2023_Ferrum_V10 %>%
  filter(!is.na(New_Grouping))
...
## Sample Count for Figure 2 and Figure 3
``` {r}
...
selected_columns <- c(4,17:106)

# Create count table
group_counts <- working_Ferrum_data %>%
  dplyr::select(New_Grouping, all_of(selected_columns)) %>%  # Select Site_Grouping + target variables
  pivot_longer(cols = -New_Grouping, names_to = "Variable", values_to = "Value") %>%  # Convert wide to long format
  filter(!is.na(Value)) %>%  # Remove NA values
  group_by(New_Grouping, Variable) %>%
...
group_counts <- group_counts %>%
  dplyr::select(New_Grouping, all_of(columns_to_move), everything())
...
group_counts_wide <- group_counts %>%
  pivot_longer(
    cols = -New_Grouping,        # all columns except New_Grouping
    names_to  = "Variable",      # old headers → Variable
    values_to = "value"          # cell values
  ) %>%
  pivot_wider(
    names_from  = New_Grouping,  # New_Grouping values become new headers
    values_from = value          # fill cells with the values
  )

# Save as an excel file
write_xlsx(group_counts_wide, path = "C:/Users/tevinger/Downloads/New_Groupings Sample Count_wide.xlsx")
```
...
# Save as an excel file
write_xlsx(group_counts, path = "C:/Users/tevinger/Downloads/New_Groupings Sample Count.xlsx")
```

## Mean and Std Dev - DELETE THIS
...
write_xlsx(group_stats, path = "C:/Users/tevinger/Downloads/Proximity Grouping Stats.xlsx")
```
...
## Sample count
```{r}
watershed_group_count <- working_Ferrum_data %>%
  group_by(New_Groups, Watershed) %>%
  summarise(N = n(), .groups = "drop")  # Count observations

group_count <- working_Ferrum_data %>%
  group_by(New_Groups) %>%
  summarise(N = n(), .groups = "drop")  # Count observations

print(watershed_group_count)
print(group_count)

watershed_sites_count <- working_Ferrum_data %>%
  group_by(Watershed, Field_label) %>%
  summarise(N = n(), .groups = "drop")

write_xlsx(watershed_sites_count, "C:/Users/tevinger/Downloads/count of sites per watershed.xlsx")

watershed_sites_groups_count <- working_Ferrum_data %>%
  group_by(Watershed, New_Grouping) %>%
  summarise(N = n(), .groups = "drop")

write_xlsx(watershed_sites_groups_count, "C:/Users/tevinger/Downloads/count of group per watershed.xlsx")
```
...
pH_plot_jitter <- ggplot(pH_long, aes(x = New_Groups, y = Conc)) +
  geom_boxplot(
    aes(color = New_Groups),          # outline color
  ...
  scale_color_manual(values = boxplot_colors) +   # boxplot outlines
  scale_shape_manual(values = group_shapes) +     # point shapes by New_Groups
  labs(
    title = NULL,
    x = NULL,
    y = "pH"
...
ggsave("C:/Users/tevinger/Box/Poulin Lab ETOX/Project Folders/Alaska BITE_HEAT (Taylor Evinger)/Evinger Manuscripts/Ferrum Manuscript/Figures/02_Global Analysis/NEW pH_with jitter points with legend.png", pH_plot_jitter, width = 5, height = 4, dpi = 300)
```
...
#ggsave("C:/Users/tevinger/Box/Poulin Lab ETOX/Project Folders/Alaska BITE_HEAT (Taylor Evinger)/Evinger Manuscripts/Ferrum Manuscript/Figures/02_Global Analysis/pH_no labels_colloquium.png", pH_plot_no_jitter, width = 2.5, height = 4, dpi = 300)
```
...
###seep and impaired tribs only
```{r}
pH_ARD_inputs_df <- pH_long %>%
  filter(New_Grouping %in% c("Seep", "Impaired Tributary"))
```

```{r}
pH_ARD_inputs_bp <- ggplot(pH_ARD_inputs_df, aes(x = New_Groups, y = Conc)) +
  geom_boxplot(aes(fill = New_Groups), width = 0.75,
               outlier.shape = 16,
               outlier.color = "black",
               outlier.size = 2,
  ...
  labs(
   ...
   scale_y_continuous(limits = c(2.5,7.6),
                     breaks = c(3,4,5,6,7,8)) +
  annotation_logticks(sides = "l", size = 1.5) +
  theme_minimal() +
  boxplot_theme +
  ...
  scale_x_discrete(expand = expansion(mult = c(0.6, 0.5)))
...
ggsave("C:/Users/tevinger/Box/Poulin Lab ETOX/Project Folders/Alaska BITE_HEAT (Taylor Evinger)/Evinger Manuscripts/Ferrum Manuscript/Figures/02_Global Analysis/ETX Seminar Additional Figs/pH_no labels.png", pH_ARD_inputs_bp, width = 2.5, height = 4, dpi = 300)
```
...
ggsave("C:/Users/tevinger/Box/Poulin Lab ETOX/Project Folders/Alaska BITE_HEAT (Taylor Evinger)/Evinger Manuscripts/Ferrum Manuscript/Figures/02_Global Analysis/SO4_no labels_colloquium.png", f_SO4_mg_per_l_plot, width = 2.5, height = 4, dpi = 300)

```

```{r}
SO4_plot_jitter <- ggplot(SO4_long, aes(x = New_Groups, y = Conc)) +
  geom_boxplot(
    aes(color = New_Groups),          # outline color
    fill = NA,                        # no fill (outline-only)
    width = 0.75,
...
ggsave("C:/Users/tevinger/Box/Poulin Lab ETOX/Project Folders/Alaska BITE_HEAT (Taylor Evinger)/Evinger Manuscripts/Ferrum Manuscript/Figures/02_Global Analysis/NEW SO4_with jitter points.png", SO4_plot_jitter, width = 2.5, height = 4, dpi = 300)
```

###seep and impaired tribs only
```{r}
SO4_ARD_inputs_df <- SO4_long %>%
  filter(New_Grouping %in% c("Seep", "Impaired Tributary"))
```

```{r}
SO4_ARD_inputs_bp <- ggplot(SO4_ARD_inputs_df, aes(x = New_Groups, y = Conc)) +
  geom_boxplot(aes(fill = New_Groups), width = 0.75, outlier.shape = 16,
               outlier.color = "black",
               outlier.size = 2, alpha = 0.85,
               size = 0.75) +
...
ggsave("C:/Users/tevinger/Box/Poulin Lab ETOX/Project Folders/Alaska BITE_HEAT (Taylor Evinger)/Evinger Manuscripts/Ferrum Manuscript/Figures/02_Global Analysis/ETX Seminar Additional Figs/SO4_no labels.png", SO4_ARD_inputs_bp, width = 2.5, height = 4, dpi = 300)
```

## SpC
```{r}
# Select DOC data
SpC_df <- Final_groups_df %>%
...
ggsave("C:/Users/tevinger/Box/Poulin Lab ETOX/Project Folders/Alaska BITE_HEAT (Taylor Evinger)/Evinger Manuscripts/Ferrum Manuscript/Figures/02_Global Analysis/SpC_log labels.png", SpC_plot, width = 1.5, height = 3.5, dpi = 300)
```

## DO
```{r}
# Select DOC data
DO_df <- Final_groups_df %>%
  dplyr::select(SamplingEventID, Watershed, New_Groups, New_Grouping, Diss_oxy_mg_per_l)

# Filter NA values in the grouping variable and make long format
DO_long <- DO_df %>%
  filter(!(New_Groups == "NA")) %>%
  pivot_longer(
    cols = c(Diss_oxy_mg_per_l),
    names_to = "Variable",
    #names_pattern = "^(.)_mgC_per_l",
    values_to = "Conc"
  ) %>%
...
```{r}
DO_plot <- ggplot(DO_long, aes(x = New_Groups, y = Conc)) +
  geom_boxplot(aes(fill = New_Groups), width = 0.75, outlier.shape = 16,
               outlier.color = "black",
               outlier.size = 2, alpha = 0.85) +
  #geom_jitter(aes(fill = Watershed), 
...
ggsave("C:/Users/tevinger/Box/Poulin Lab ETOX/Project Folders/Alaska BITE_HEAT (Taylor Evinger)/Evinger Manuscripts/Ferrum Manuscript/Figures/02_Global Analysis/DO_with labels.png", DO_plot, width = 1.5, height = 3.5, dpi = 300)
```
...
ggsave("C:/Users/tevinger/Box/Poulin Lab ETOX/Project Folders/Alaska BITE_HEAT (Taylor Evinger)/Evinger Manuscripts/Ferrum Manuscript/Figures/02_Global Analysis/Cl_no labels.png", Cl_plot, width = 1.5, height = 3.5, dpi = 300)
```

## DIC
```{r}
# Select DOC data
DIC_df <- Final_groups_df %>%
...
ggsave("C:/Users/tevinger/Box/Poulin Lab ETOX/Project Folders/Alaska BITE_HEAT (Taylor Evinger)/Evinger Manuscripts/Ferrum Manuscript/Figures/02_Global Analysis/DIC_no labels.png", DIC_plot, width = 1.5, height = 3.5, dpi = 300)
```
...
ggsave("C:/Users/tevinger/Box/Poulin Lab ETOX/Project Folders/Alaska BITE_HEAT (Taylor Evinger)/Evinger Manuscripts/Ferrum Manuscript/Figures/02_Global Analysis/Alk_no labels.png", Alk_plot, width = 1.5, height = 3.5, dpi = 300)
...
ggsave("C:/Users/tevinger/Box/Poulin Lab ETOX/Project Folders/Alaska BITE_HEAT (Taylor Evinger)/Evinger Manuscripts/Ferrum Manuscript/Figures/02_Global Analysis/DOC_no labels.png", DOC_plot, width = 1.5, height = 3.5, dpi = 300)
...
```{r}
# Select alkalinity data
Ca_df <- Alaska_DataRelease_2022_2023_Ferrum_V9 %>%
...
ggsave("C:/Users/tevinger/Box/Poulin Lab ETOX/Project Folders/Alaska BITE_HEAT (Taylor Evinger)/Evinger Manuscripts/Ferrum Manuscript/Figures/02_Global Analysis/Ca_no labels.png", Ca_plot, width = 1.5, height = 3.5, dpi = 300)
...
ggsave("C:/Users/tevinger/Box/Poulin Lab ETOX/Project Folders/Alaska BITE_HEAT (Taylor Evinger)/Evinger Manuscripts/Ferrum Manuscript/Figures/02_Global Analysis/Mg_no labels.png", Mg_plot, width = 1.5, height = 3.5, dpi = 300)
```

##K
```{r}
# Select potassium data
K_df <- Final_groups_df %>%
  dplyr::select(SamplingEventID, Watershed, New_Groups, New_Grouping, f_K_mg_l)
...
K_breaks <- seq(0, 1000, by = 100)
K_labels <- c(0,"",200,"",400,"",600,"",800,"",1000)
K_labels

```

```{r}
K_plot <- ggplot(K_long, aes(x = New_Groups, y = Conc)) +
  geom_boxplot(aes(fill = New_Groups), width = 0.75, outlier.shape = 16,
               outlier.color = "black",
               outlier.size = 2, alpha = 0.85) +
  scale_fill_manual(values = boxplot_colors) +
  ...
  #               breaks = c(5, 10, 100, 1000)) +
  # annotation_logticks(sides = "l") +
  #scale_y_continuous(breaks = K_breaks, labels = K_labels) +
  theme_minimal() +
  boxplot_theme +
  theme(
    legend.position = "none",
...
ggsave("C:/Users/tevinger/Box/Poulin Lab ETOX/Project Folders/Alaska BITE_HEAT (Taylor Evinger)/Evinger Manuscripts/Ferrum Manuscript/Figures/02_Global Analysis/K no labels.png", K_plot, width = 1.5, height = 3.5, dpi = 300)
```

##Na
```{r}
# Select Na data
Na_df <- Final_groups_df %>%
  dplyr::select(SamplingEventID, Watershed, New_Groups, New_Grouping, f_Na_mg_l)
...
```{r}
Na_plot <- ggplot(Na_long, aes(x = New_Groups, y = Conc)) +
  geom_boxplot(aes(fill = New_Groups), width = 0.75, outlier.shape = 16,
               outlier.color = "black",
               outlier.size = 2, alpha = 0.85) +
  #geom_jitter(aes(fill = Watershed), 
...
ggsave(
  "C:/Users/tevinger/Box/Poulin Lab ETOX/Project Folders/Alaska BITE_HEAT (Taylor Evinger)/Evinger Manuscripts/Ferrum Manuscript/Figures/02_Global Analysis/Na_no labels.png",
  Na_plot,
  width = 1.5,
  height = 3.5,
  dpi = 300
...
ggsave("C:/Users/tevinger/Box/Poulin Lab ETOX/Project Folders/Alaska BITE_HEAT (Taylor Evinger)/Evinger Manuscripts/Ferrum Manuscript/Figures/02_Global Analysis/Fe_no labels_colloquium.png", Fe_plot, width = 3.25, height = 4, dpi = 300)
```
...
```{r}
Fe_plot_jitter <- ggplot(Fe_long, aes(x = x_group, y = Metal_conc_plot)) +
  geom_boxplot_pattern(
    aes(
      color = New_Groups,                 # outline color by group
      pattern = Metal_type,
      pattern_fill = Metal_type
    ),
    fill = NA,                            # outline-only boxplots
    outlier.shape = NA,                   # hide boxplot outliers (we'll show points)
    outlier.color = "black",
    outlier.size = 2,
  ...
  scale_pattern_manual(values = c("f" = "none", "p" = "stripe"), name = "Metal Type") +
  scale_pattern_fill_manual(values = c("f" = NA, "p" = "grey40")) +
  scale_y_log10(
    limits = c(0.7, 1500000),
    labels = scales::comma,
    breaks = c(1, 10, 100, 1000, 10000, 100000, 1000000),
...
ggsave("C:/Users/tevinger/Box/Poulin Lab ETOX/Project Folders/Alaska BITE_HEAT (Taylor Evinger)/Evinger Manuscripts/Ferrum Manuscript/Figures/02_Global Analysis/NEW Fe_jitter points.png", Fe_plot_jitter, width = 3.25, height = 4, dpi = 300)
```

##Seep and impaired tributary only
...
ggsave("C:/Users/tevinger/Box/Poulin Lab ETOX/Project Folders/Alaska BITE_HEAT (Taylor Evinger)/Evinger Manuscripts/Ferrum Manuscript/Figures/02_Global Analysis/Zn_no labels_colloquium.png", Zn_plot, width = 3.25, height = 4, dpi = 300)
```
...
# Net Difference
Net difference between upstream and downstream mainstem site groups in each watershed
if a site group had more than one observation, they were averaged and then used for the net difference calculation

## Alk
```{r}
# select only upstream and downstream site groups
Alk_up_and_down_MS <- Alk_long %>%
  filter(New_Grouping %in% c("Upstream", "Downstream MS"))

Alk_up_and_down_MS_means <- Alk_up_and_down_MS %>%
  group_by(Watershed, New_Grouping) %>%
...
```{r}
# Calculate the net difference for each watershed

Alk_net_difference <- Alk_up_and_down_MS_means %>%
  ...
  mutate(
    net_difference = `Downstream MS` - `Upstream`,
    percent_change  = ((`Downstream MS` - `Upstream`) / `Upstream`) *100
  ) %>%
  ungroup() %>%
  mutate(
    interpretation = case_when(
      net_difference < 0  ~ "reduced",
      net_difference > 0  ~ "enriched",
      net_difference == 0 ~ "no change",
      TRUE ~ NA_character_
    ),
    Parameter = "Alk"
  )

view(Alk_net_difference)
```

```{r}
# plot
Alk_net_diff_p <- Alk_net_difference %>%
  ggplot(aes(x = Parameter, y = net_difference)) +
  geom_hline(
    yintercept = 0,
    linetype = "solid",
    color = "darkgrey",
  ...
  labs(
    title = NULL,
    y = NULL,
    # y = "Net Difference in Mainstem Upstream to Downstream",
    fill = "Metal Type"
  ) +
  theme_minimal() +
  theme(
    ...
    axis.title.y = element_text(size = 16, color = "black", face = "bold",
                                vjust = 1.25, margin = margin(r = 10)),
    legend.position = "none",
    legend.background = element_rect(fill = NA),
    legend.text = element_text(size = 12, color = "black", face = "bold"),
    legend.box.background = element_rect(fill = NA, color = "black", size = 0.75),
    axis.ticks.y = element_line(size = 0.9, color = "black"),
...
#ggsave("C:/Users/tevinger/Box/Poulin Lab ETOX/Project Folders/Alaska BITE_HEAT (Taylor Evinger)/Evinger Manuscripts/Ferrum Manuscript/Figures/02_Global Analysis/sulfate net difference.png", SO4_net_diff_p, width = 3.25, height = 4, dpi = 300)
```

```{r}
Alk_net_diff_change <- Alk_net_difference %>%
  ggplot(aes(x = Parameter, y = percent_change)) +
  geom_hline(
    yintercept = 0,
    linetype = "solid",
    color = "darkgrey",
  ...
  labs(
    title = NULL,
    y = NULL,
    # y = "Percent Change in Mainstem Upstream to Downstream",
    fill = "Metal Type"
  ) +
  coord_cartesian(ylim = c(-100,168000)) +
...
```{r}
# select only upstream and downstream site groups
SO4_up_and_down_MS <- SO4_long %>%
  filter(New_Grouping %in% c("Upstream", "Downstream MS"))

SO4_up_and_down_MS_means <- SO4_up_and_down_MS %>%
  group_by(Watershed, New_Grouping) %>%
  summarise(mean_value = mean(Conc, na.rm = TRUE),
            num_observations = n(),
            .groups = "drop")
...
filtered_SO4_net_difference <- SO4_up_and_down_MS_means %>%
  select(-num_observations) %>%
  #filter(Metal_type == "f") %>%
  group_by(Watershed) %>%
  pivot_wider(
...
view(filtered_SO4_net_difference)

SO4_net_difference <- filtered_SO4_net_difference
```

```{r}
# plot
SO4_net_diff_p <- SO4_net_difference %>%
  ggplot(aes(x = Parameter, y = net_difference)) +
  geom_hline(
    yintercept = 0,
    linetype = "solid",
...
```{r}
SO4_net_diff_change <- SO4_net_difference %>%
  ggplot(aes(x = Parameter, y = percent_change)) +
  geom_hline(
    yintercept = 0,
    linetype = "solid",
...
```{r}
# select only upstream and downstream site groups
Fe_up_and_down_MS <- Fe_long %>%
  filter(New_Grouping %in% c("Upstream", "Downstream MS"))

Fe_up_and_down_MS_means <- Fe_up_and_down_MS %>%
  group_by(Watershed, New_Grouping, Metal_type) %>%
  summarise(mean_value = mean(Metal_conc, na.rm = TRUE),
            num_observations = n(),
    .groups = "drop")
```
...
filtered_Fe_net_difference <- Fe_up_and_down_MS_means %>%
  select(-num_observations) %>%
  filter(Metal_type == "f") %>%
  group_by(Watershed) %>%
  pivot_wider(
...
# view(filtered_Fe_net_difference)

particulate_Fe_net_difference <- Fe_up_and_down_MS_means %>%
  select(-num_observations) %>%
  filter(Metal_type == "p") %>%
  group_by(Watershed) %>%
  pivot_wider(
  ...
  mutate(
    upstream_adj = if_else(
      `Upstream` == 0,
      sqrt(2) / 2 * 1,
      `Upstream`
    ),
    net_difference = `Downstream MS` - `upstream_adj`,
    percent_change  = ((`Downstream MS` - `upstream_adj`) / `upstream_adj`) *100
  ) %>%
  ungroup() %>%
  mutate(
    interpretation = case_when(
...
view(particulate_Fe_net_difference)
```

```{r}
# combine results for filtered and particulate into one df
Fe_net_difference <- bind_rows(filtered_Fe_net_difference, particulate_Fe_net_difference)

# view(Fe_net_difference)
```

```{r}
# plot
Fe_net_diff_p <- Fe_net_difference %>%
  ggplot(aes(x = Metal_type, y = net_difference)) +
  geom_hline(
    yintercept = 0,
    linetype = "solid",
    color = "darkgrey",
  ...
  # axis.ticks.x = element_blank(),
  axis.text.y = element_text(size = 14, color = "black", face = "bold"),
  axis.title.y = element_text(size = 16, color = "black", face = "bold", vjust = 1.25, margin = margin(r = 10)),
  legend.position = "top",
  legend.title = element_blank(),
  legend.background = element_rect(fill = NA),
  legend.text = element_text(size = 12, color = "black", face = "bold"),
...
#ggsave("C:/Users/tevinger/Box/Poulin Lab ETOX/Project Folders/Alaska BITE_HEAT (Taylor Evinger)/Evinger Manuscripts/Ferrum Manuscript/Figures/02_Global Analysis/Zn_no labels_colloquium.png", Fe_net_diff_p, width = 3.25, height = 4, dpi = 300)
```

```{r}
Fe_net_diff_change <- Fe_net_difference %>%
  ggplot(aes(x = Metal_type, y = percent_change)) +
  geom_rect(
  xmin = 1.5, 
  xmax = Inf,
  ymin = -Inf,
...
```{r}
# select only upstream and downstream site groups
Ni_up_and_down_MS <- Ni_long %>%
  filter(New_Grouping %in% c("Upstream", "Downstream MS"))

Ni_up_and_down_MS_means <- Ni_up_and_down_MS %>%
  group_by(Watershed, New_Grouping, Metal_type) %>%
  summarise(mean_value = mean(Metal_conc, na.rm = TRUE),
            num_observations = n(),
            .groups = "drop")
...
filtered_Ni_net_difference <- Ni_up_and_down_MS_means %>%
  select(-num_observations) %>%
  filter(Metal_type == "f") %>%
  group_by(Watershed) %>%
  pivot_wider(
...
view(filtered_Ni_net_difference)

particulate_Ni_net_difference <- Ni_up_and_down_MS_means %>%
  select(-num_observations) %>%
  filter(Metal_type == "p") %>%
  group_by(Watershed) %>%
  pivot_wider(
  ...
  mutate(
    ...
    downstream_adj = if_else(
      `Downstream MS` == 0,
      sqrt(2) / 2 * .1,
      `Downstream MS`
    ),
    net_difference = `downstream_adj` - `Upstream`,
    percent_change  = ((`downstream_adj` - `upstream_adj`) / `upstream_adj`) *100
  ) %>%
  ungroup() %>%
  mutate(
    interpretation = case_when(
...
view(particulate_Ni_net_difference)
```

```{r}
# combine results for filtered and particulate into one df
Ni_net_difference <- bind_rows(filtered_Ni_net_difference, particulate_Ni_net_difference)

view(Ni_net_difference)
```

```{r}
# plot
Ni_net_diff_p <- Ni_net_difference %>%
  ggplot(aes(x = Metal_type, y = net_difference)) +
  geom_rect(
  xmin = 1.5, 
  xmax = Inf,
  ...
  geom_point(
    ...
    size = 3.3,
    stroke = 0.6,         # outline thickness
    position = position_jitter(width = 0.07, height = 0.05)
  ) +
  scale_fill_manual(values = watershed_colors) +
  labs(
    #title = "Method 3 - Ni",
  ...
  coord_cartesian(ylim = c(-60,168000)) +
  scale_y_continuous(trans = scales::pseudo_log_trans(base = 10),
                     breaks = c(
    -10000, -5000, -1000, -500, -100,
    -50, -10, -5, -1,
     0,
     1, 5, 10, 50, 100, 500, 1000, 5000, 10000, 100000, 300000
  ),
  labels = scales::label_number()) +
  theme_minimal() +
  theme(
...
```{r}
# plot
Ni_net_diff_change <- Ni_net_difference %>%
  ggplot(aes(x = Metal_type, y = percent_change)) +
  geom_rect(
  xmin = 1.5, 
  xmax = Inf,
...
```{r}
# select only upstream and downstream site groups
Zn_up_and_down_MS <- Zn_long %>%
  filter(New_Grouping %in% c("Upstream", "Downstream MS"))

Zn_up_and_down_MS_means <- Zn_up_and_down_MS %>%
  group_by(Watershed, New_Grouping, Metal_type) %>%
  summarise(
    mean_value = mean(Metal_conc, na.rm = TRUE),
    num_observations = n(),
    .groups = "drop"
  )
```
...
filtered_Zn_net_difference <- Zn_up_and_down_MS_means %>%
  select(-num_observations) %>%
  filter(Metal_type == "f") %>%
  group_by(Watershed) %>%
  pivot_wider(
...
view(filtered_Zn_net_difference)

particulate_Zn_net_difference <- Zn_up_and_down_MS_means %>%
  select(-num_observations) %>%
  filter(Metal_type == "p") %>%
  group_by(Watershed) %>%
  pivot_wider(
...
view(particulate_Zn_net_difference)
```

```{r}
# combine results for filtered and particulate into one df
Zn_net_difference <- bind_rows(
  filtered_Zn_net_difference,
  particulate_Zn_net_difference
)

view(Zn_net_difference)
```

```{r}
# plot
Zn_net_diff_p <- Zn_net_difference %>%
  ggplot(aes(x = Metal_type, y = net_difference)) +
  geom_hline(
    yintercept = 0,
    linetype = "solid",
...
```{r}
# plot
Zn_net_diff_change <- Zn_net_difference %>%
  ggplot(aes(x = Metal_type, y = percent_change)) +
  geom_rect(
  xmin = 1.5, 
  xmax = Inf,
...
```{r}
# select only upstream and downstream site groups
Cd_up_and_down_MS <- Cd_long %>%
  filter(New_Grouping %in% c("Upstream", "Downstream MS"))

Cd_up_and_down_MS_means <- Cd_up_and_down_MS %>%
  group_by(Watershed, New_Grouping, Metal_type) %>%
  summarise(
    mean_value = mean(Metal_conc, na.rm = TRUE),
    num_observations = n(),
...
filtered_Cd_net_difference <- Cd_up_and_down_MS_means %>%
  select(-num_observations) %>%
  filter(Metal_type == "f") %>%
  group_by(Watershed) %>%
  pivot_wider(
...
# view(filtered_Cd_net_difference)

particulate_Cd_net_difference <- Cd_up_and_down_MS_means %>%
  select(-num_observations) %>%
  filter(Metal_type == "p") %>%
  group_by(Watershed) %>%
  pivot_wider(
...
view(particulate_Cd_net_difference)
```

```{r}
# combine results for filtered and particulate into one df
Cd_net_difference <- bind_rows(filtered_Cd_net_difference, particulate_Cd_net_difference)

# view(Cd_net_difference)
```

```{r}
# plot
Cd_net_diff_p <- Cd_net_difference %>%
  ggplot(aes(x = Metal_type, y = net_difference)) +
  geom_hline(
    yintercept = 0,
    linetype = "solid",
...
```{r}
Cd_net_diff_change <- Cd_net_difference %>%
  ggplot(aes(x = Metal_type, y = percent_change)) +
  geom_rect(
    xmin = 1.5,
    xmax = Inf,
...
```{r}
# select only upstream and downstream site groups
Al_up_and_down_MS <- Al_long %>%
  filter(New_Grouping %in% c("Upstream", "Downstream MS"))

Al_up_and_down_MS_means <- Al_up_and_down_MS %>%
  group_by(Watershed, New_Grouping, Metal_type) %>%
  summarise(
    mean_value = mean(Metal_conc, na.rm = TRUE),
    num_observations = n(),
...
filtered_Al_net_difference <- Al_up_and_down_MS_means %>%
  select(-num_observations) %>%
  filter(Metal_type == "f") %>%
  group_by(Watershed) %>%
  pivot_wider(
...
# view(filtered_Al_net_difference)

particulate_Al_net_difference <- Al_up_and_down_MS_means %>%
  select(-num_observations) %>%
  filter(Metal_type == "p") %>%
  group_by(Watershed) %>%
  pivot_wider(
...
view(particulate_Al_net_difference)
```

```{r}
# combine results for filtered and particulate into one df
Al_net_difference <- bind_rows(filtered_Al_net_difference,
                               particulate_Al_net_difference)

# view(Al_net_difference)
```

```{r}
# plot (net difference)
Al_net_diff_p <- Al_net_difference %>%
  ggplot(aes(x = Metal_type, y = net_difference)) +
  geom_hline(
    yintercept = 0,
    linetype = "solid",
...
```{r}
# plot (percent change; pseudo-log)
Al_net_diff_change <- Al_net_difference %>%
  ggplot(aes(x = Metal_type, y = percent_change)) +
  geom_rect(
    xmin = 1.5,
    xmax = Inf,
...
```{r}
# select only upstream and downstream site groups
REE_up_and_down_MS <- REE_long %>%
  filter(New_Grouping %in% c("Upstream", "Downstream MS"))

REE_up_and_down_MS_means <- REE_up_and_down_MS %>%
  group_by(Watershed, New_Grouping, Metal_type) %>%
  summarise(
    mean_value = mean(Metal_conc, na.rm = TRUE),
    num_observations = n(),
...
filtered_REE_net_difference <- REE_up_and_down_MS_means %>%
  select(-num_observations) %>%
  filter(Metal_type == "f") %>%
  group_by(Watershed) %>%
  pivot_wider(
...
# view(filtered_REE_net_difference)

particulate_REE_net_difference <- REE_up_and_down_MS_means %>%
  select(-num_observations) %>%
  filter(Metal_type == "p") %>%
  group_by(Watershed) %>%
  pivot_wider(
...
view(particulate_REE_net_difference)
```

```{r}
# combine results for filtered and particulate into one df
REE_net_difference <- bind_rows(filtered_REE_net_difference,
                                particulate_REE_net_difference)

# view(REE_net_difference)
```

```{r}
# plot (net difference)
REE_net_diff_p <- REE_net_difference %>%
  ggplot(aes(x = Metal_type, y = net_difference)) +
  geom_hline(
    yintercept = 0,
    linetype = "solid",
...
```{r}
# plot (percent change; pseudo-log)
REE_net_diff_change <- REE_net_difference %>%
  ggplot(aes(x = Metal_type, y = percent_change)) +
  geom_rect(
    xmin = 1.5,
    xmax = Inf,
...
## combined plot
```{r}
SO4_p <- SO4_net_diff_p + theme(axis.text.x = element_blank(), axis.text.y = element_blank(), axis.title.y = element_blank(), plot.margin = margin(5, 15, 5, 35))

Fe_p <- Fe_net_diff_p + theme(axis.text.x = element_blank(), axis.text.y = element_blank(), axis.title.y = element_blank(), plot.margin = margin(5, 15, 5, 35))
# Fe_p

Ni_p <- Ni_net_diff_p + theme(axis.text.x = element_blank(), axis.text.y = element_blank(), axis.title.y = element_blank(), plot.margin = margin(5, 15, 5, 35))
# Ni_p

Zn_p <- Zn_net_diff_p + theme(axis.text.x = element_blank(), axis.text.y = element_blank(), axis.title.y = element_blank(), plot.margin = margin(5, 15, 5, 35))
# Zn_p

Cd_p <- Cd_net_diff_p + theme(axis.text.x = element_blank(), axis.text.y = element_blank(), axis.title.y = element_blank(), plot.margin = margin(5, 15, 5, 35))


Al_p <- Al_net_diff_p + theme(axis.text.x = element_blank(), axis.text.y = element_blank(), axis.title.y = element_blank(), plot.margin = margin(5, 15, 5, 35))


REE_p <- REE_net_diff_p + theme(axis.text.x = element_blank(), axis.text.y = element_blank(), axis.title.y = element_blank(), plot.margin = margin(5, 15, 5, 35))

```

```{r}
Alk_p <- Alk_net_diff_change + theme(axis.text.x = element_blank(), axis.text.y = element_blank(), axis.title.y = element_blank(), plot.margin = margin(5, 35, 5, 5))

SO4_p <- SO4_net_diff_change + theme(axis.text.x = element_blank(), axis.text.y = element_blank(), axis.title.y = element_blank(), plot.margin = margin(5, 35, 5, 5))

Fe_p <- Fe_net_diff_change + theme(axis.text.x = element_blank(), axis.text.y = element_blank(), axis.title.y = element_blank(), plot.margin = margin(5, 35, 5, 5))
# Fe_p

Ni_p <- Ni_net_diff_change + theme(axis.text.x = element_blank(), axis.text.y = element_blank(), axis.title.y = element_blank(), plot.margin = margin(5, 35, 5, 5))
# Ni_p

Zn_p <- Zn_net_diff_change + theme(axis.text.x = element_blank(), axis.text.y = element_blank(), axis.title.y = element_blank(), plot.margin = margin(5, 5, 5, 5))
# Zn_p

Cd_p <- Cd_net_diff_change + theme(axis.text.x = element_blank(), axis.text.y = element_blank(), axis.title.y = element_blank(), plot.margin = margin(5, 35, 5, 5))

Al_p <- Al_net_diff_change + theme(axis.text.x = element_blank(), axis.text.y = element_blank(), axis.title.y = element_blank(), plot.margin = margin(5, 5, 5, 5))

REE_p <- REE_net_diff_change + theme(axis.text.x = element_blank(), axis.text.y = element_blank(), axis.title.y = element_blank(), plot.margin = margin(5, 35, 5, 5))
```

```{r}
library(patchwork)

top_net_difference_p <- Alk_p + SO4_p + Fe_p + Al_p +
  plot_layout(
    nrow = 1,
    ncol = 4
  )

top_net_difference_p


bottom_net_difference_p <- REE_p + Ni_p + Cd_p + Zn_p +
  plot_layout(
    nrow = 1,
    ncol = 4
  )

bottom_net_difference_p

ggsave("C:/Users/tevinger/Box/Poulin Lab ETOX/Project Folders/Alaska BITE_HEAT (Taylor Evinger)/Evinger Manuscripts/Ferrum Manuscript/Figures/02_Global Analysis/Percent Change/Alk, SO4, Fe, Al percent change in MS.png", top_net_difference_p, width = 8, height = 4, dpi = 300)

ggsave("C:/Users/tevinger/Box/Poulin Lab ETOX/Project Folders/Alaska BITE_HEAT (Taylor Evinger)/Evinger Manuscripts/Ferrum Manuscript/Figures/02_Global Analysis/Percent Change/REE, Ni, Cd, Zn percent change in MS.png", bottom_net_difference_p, width = 8, height = 4, dpi = 300)
```


## net difference funciton and plot
```{r}
...
```{r}
# Select Mn data
Mn_df <- Alaska_DataRelease_2022_2023_Ferrum_V9 %>%
  dplyr::select(SamplingEventID, Watershed, New_Groups, New_Grouping, f_Mn_mcg_per_l, f_Mn_mcg_per_l_qa, p_Mn_mcg_per_l, p_Mn_mcg_per_l_qa) %>%
  filter(!(is.na(New_Grouping)))

# Filter NA values in the grouping variable, filter particulate concentrations and make long format
Mn_long <- Mn_df %>%
  pivot_longer(
    cols = c(f_Mn_mcg_per_l, p_Mn_mcg_per_l),
    names_to = "Metal_type",
...
```{r}
Mn_plot <- ggplot(Mn_long, aes(x = x_group, y = Metal_conc_plot)) +
  geom_boxplot_pattern(
    aes(fill = New_Groups, pattern = Metal_type, pattern_fill = Metal_type),
    outlier.shape = 16,
    outlier.color = "black",
  ...
  labs(
    title = NULL,
    x = NULL,
    y = ylab_Mn,  # make sure you define `ylab_Mn` beforehand
    fill = "Metal Type"
  ) +
  theme_minimal() +
  boxplot_theme +
...
#ggsave("C:/Users/tevinger/Box/Poulin Lab ETOX/Project Folders/Alaska BITE_HEAT (Taylor Evinger)/Evinger Manuscripts/Ferrum Manuscript/Figures/02_Global Analysis/Mn_V2 no labels.png", Mn_plot, width = 2.75, height = 3.25, dpi = 300)
```
...
#ggsave("C:/Users/tevinger/Box/Poulin Lab ETOX/Project Folders/Alaska BITE_HEAT (Taylor Evinger)/Evinger Manuscripts/Ferrum Manuscript/Figures/02_Global Analysis/Cd_V2 no labels.png", Cd_plot, width = 2.75, height = 3.25, dpi = 300)
```

##Co
```{r}
# Select Co data
Co_df <- Alaska_DataRelease_2022_2023_Ferrum_V9 %>%
  dplyr::select(SamplingEventID, Watershed, New_Groups, New_Grouping, 
...
Co_long <- Co_df %>%
  ...
  filter(!is.na(Metal_conc)) %>% # filtering out here means only the f or p value that is NA is excluded, not both measurements for the same sample
  mutate(Metal_conc_plot = case_when(
  Metal_conc == 0 ~ Metal_conc + ((sqrt(2)/2) * 0.01),
  Metal_conc < ((sqrt(2)/2) * 0.01) ~ Metal_conc + ((sqrt(2)/2) * 0.01),
  TRUE ~ Metal_conc
))
...
Co_below_DL <- Co_long %>%
  filter(Metal_conc_plot < ((sqrt(2)/2)*0.01))
...
#ggsave("C:/Users/tevinger/Box/Poulin Lab ETOX/Project Folders/Alaska BITE_HEAT (Taylor Evinger)/Evinger Manuscripts/Ferrum Manuscript/Figures/02_Global Analysis/Co_V2 no labels.png", Co_plot, width = 2.75, height = 3.25, dpi = 300)
```

## Se 
```{r}
# Select Se data
Se_df <- Alaska_DataRelease_2022_2023_Ferrum_V9 %>%
  dplyr::select(SamplingEventID, Watershed, New_Groups, New_Grouping, 
...
#ggsave("C:/Users/tevinger/Box/Poulin Lab ETOX/Project Folders/Alaska BITE_HEAT (Taylor Evinger)/Evinger Manuscripts/Ferrum Manuscript/Figures/02_Global Analysis/Se_V1 with labels.png", Se_plot, width = 2.75, height = 3.25, dpi = 300)
```

## Ba
```{r}
# Select Ba data
Ba_df <- Alaska_DataRelease_2022_2023_Ferrum_V9 %>%
  dplyr::select(SamplingEventID, Watershed, New_Groups, New_Grouping, f_Ba_mcg_per_l, f_Ba_mcg_per_l_qa, p_Ba_mcg_per_l, p_Ba_mcg_per_l_qa) %>%
  filter(!(is.na(New_Grouping)))

# Filter NA values in the grouping variable, convert to long format and handle low/zero values
Ba_long <- Ba_df %>%
  pivot_longer(
    cols = c(f_Ba_mcg_per_l, p_Ba_mcg_per_l),
    names_to = "Metal_type",
...
```{r}
Ba_plot <- ggplot(Ba_long, aes(x = x_group, y = Metal_conc_plot)) +
  geom_boxplot_pattern(
    aes(fill = New_Groups, pattern = Metal_type, pattern_fill = Metal_type),
    outlier.shape = 16,
    outlier.color = "black",
  ...
  labs(
    title = NULL,
    x = NULL,
    y = "Ba",  # define this before running, e.g., ylab_Ba <- "Barium concentration (µg/L)"
    fill = "Metal Type"
  ) +
  theme_minimal() +
  boxplot_theme +
...
ggsave("C:/Users/tevinger/Box/Poulin Lab ETOX/Project Folders/Alaska BITE_HEAT (Taylor Evinger)/Evinger Manuscripts/Ferrum Manuscript/Figures/02_Global Analysis/Ba_V2 no labels.png", Ba_plot, width = 2.75, height = 3.25, dpi = 300)
```

## Pb
```{r}
# Select Pb data
Pb_df <- Alaska_DataRelease_2022_2023_Ferrum_V9 %>%
  dplyr::select(SamplingEventID, Watershed, New_Groups, New_Grouping, f_Pb_mcg_per_l, f_Pb_mcg_per_l_qa, p_Pb_mcg_per_l, p_Pb_mcg_per_l_qa) %>%
  filter(!(is.na(New_Grouping)))

# Filter NA values in the grouping variable, filter particulate concentrations and make long format
Pb_long <- Pb_df %>%
  pivot_longer(
    cols = c(f_Pb_mcg_per_l, p_Pb_mcg_per_l),
    names_to = "Metal_type",
    names_pattern = "^(.)_Pb_mcg_per_l",
    values_to = "Metal_conc"
  ) %>%
  filter(!is.na(Metal_conc)) %>%
  mutate(Metal_conc_plot = case_when(
...
```{r}
Pb_plot <- ggplot(Pb_long, aes(x = x_group, y = Metal_conc_plot)) +
  geom_boxplot_pattern(
    aes(fill = New_Groups, pattern = Metal_type, pattern_fill = Metal_type),
    outlier.shape = 16,
    outlier.color = "black",
  ...
  labs(
    title = NULL,
    x = NULL,
    y = ylab_Pb,  # make sure you define ylab_Pb beforehand
    fill = "Metal Type"
  ) +
  theme_minimal() +
  boxplot_theme +
...
#ggsave("C:/Users/tevinger/Box/Poulin Lab ETOX/Project Folders/Alaska BITE_HEAT (Taylor Evinger)/Evinger Manuscripts/Ferrum Manuscript/Figures/02_Global Analysis/Pb_V1 with labels.png", Pb_plot, width = 2.75, height = 3.25, dpi = 300)
```

## U
```{r}
# Select U data
U_df <- Alaska_DataRelease_2022_2023_Ferrum_V9 %>%
  dplyr::select(SamplingEventID, Watershed, New_Groups, New_Grouping, f_U_mcg_per_l, f_U_mcg_per_l_qa, p_U_mcg_per_l, p_U_mcg_per_l_qa) %>%
  filter(!(is.na(New_Grouping)))

# Filter NA values in the grouping variable, convert to long format and handle low/zero values
U_long <- U_df %>%
  pivot_longer(
    cols = c(f_U_mcg_per_l, p_U_mcg_per_l),
    names_to = "Metal_type",
    names_pattern = "^(.)_U_mcg_per_l",
    values_to = "Metal_conc"
  ) %>%
  filter(!is.na(Metal_conc)) %>%
  mutate(Metal_conc_plot = case_when(
...
```{r}
U_plot <- ggplot(U_long, aes(x = x_group, y = Metal_conc_plot)) +
  geom_boxplot_pattern(
    aes(fill = New_Groups, pattern = Metal_type, pattern_fill = Metal_type),
    outlier.shape = 16,
    outlier.color = "black",
  ...
  labs(
    title = NULL,
    x = NULL,
    y = "U",  # define this before running (e.g., ylab_U <- "Uranium concentration (µg/L)")
    fill = "Metal Type"
  ) +
  theme_minimal() +
  boxplot_theme +
...
#ggsave("C:/Users/tevinger/Box/Poulin Lab ETOX/Project Folders/Alaska BITE_HEAT (Taylor Evinger)/Evinger Manuscripts/Ferrum Manuscript/Figures/02_Global Analysis/U_V2 no labels.png", U_plot, width = 2.75, height = 3.25, dpi = 300)
```

## Tl
```{r}
# Select Tl data
Tl_df <- Alaska_DataRelease_2022_2023_Ferrum_V9 %>%
  dplyr::select(SamplingEventID, Watershed, New_Groups, New_Grouping, f_Tl_mcg_per_l, f_Tl_mcg_per_l_qa, p_Tl_mcg_per_l, p_Tl_mcg_per_l_qa) %>%
  filter(!(is.na(New_Grouping)))

# Filter NA values in the grouping variable, convert to long format and handle low/zero values
Tl_long <- Tl_df %>%
  pivot_longer(
    cols = c(f_Tl_mcg_per_l, p_Tl_mcg_per_l),
    names_to = "Metal_type",
    names_pattern = "^(.)_Tl_mcg_per_l",
    values_to = "Metal_conc"
  ) %>%
  filter(!is.na(Metal_conc)) %>%
  mutate(Metal_conc_plot = case_when(
...
```{r}
Tl_plot <- ggplot(Tl_long, aes(x = x_group, y = Metal_conc_plot)) +
  geom_boxplot_pattern(
    aes(fill = New_Groups, pattern = Metal_type, pattern_fill = Metal_type),
    outlier.shape = 16,
    outlier.color = "black",
  ...
  labs(
    title = NULL,
    x = NULL,
    y = "Tl",  # define this before running (e.g., ylab_Tl <- "Thallium concentration (µg/L)")
    fill = "Metal Type"
  ) +
  theme_minimal() +
  boxplot_theme +
...
#ggsave("C:/Users/tevinger/Box/Poulin Lab ETOX/Project Folders/Alaska BITE_HEAT (Taylor Evinger)/Evinger Manuscripts/Ferrum Manuscript/Figures/02_Global Analysis/Tl_V2 no labels.png", Tl_plot, width = 2.75, height = 3.25, dpi = 300)
```

## As
```{r}
# Select As data
As_df <- Alaska_DataRelease_2022_2023_Ferrum_V9 %>%
  dplyr::select(SamplingEventID, Watershed, New_Groups, New_Grouping, f_As_mcg_per_l, f_As_mcg_per_l_qa, p_As_mcg_per_l, p_As_mcg_per_l_qa) %>%
  filter(!(is.na(New_Grouping)))

# Filter NA values in the grouping variable, convert to long format and handle low/zero values
As_long <- As_df %>%
  pivot_longer(
    cols = c(f_As_mcg_per_l, p_As_mcg_per_l),
    names_to = "Metal_type",
    names_pattern = "^(.)_As_mcg_per_l",
    values_to = "Metal_conc"
  ) %>%
  filter(!is.na(Metal_conc)) %>%
  mutate(Metal_conc_plot = case_when(
...
```{r}
As_plot <- ggplot(As_long, aes(x = x_group, y = Metal_conc_plot)) +
  geom_boxplot_pattern(
    aes(fill = New_Groups, pattern = Metal_type, pattern_fill = Metal_type),
    outlier.shape = 16,
    outlier.color = "black",
  ...
  labs(
    title = NULL,
    x = NULL,
    y = "As",  # or use ylab_As <- "Arsenic concentration (µg/L)" if preferred
    fill = "Metal Type"
  ) +
  theme_minimal() +
  boxplot_theme +
...
#ggsave("C:/Users/tevinger/Box/Poulin Lab ETOX/Project Folders/Alaska BITE_HEAT (Taylor Evinger)/Evinger Manuscripts/Ferrum Manuscript/Figures/02_Global Analysis/As_V2 no labels.png", As_plot, width = 2.75, height = 3.25, dpi = 300)
```

## Cr
```{r}
# Select Cr data
Cr_df <- Alaska_DataRelease_2022_2023_Ferrum_V9 %>%
  dplyr::select(SamplingEventID, Watershed, New_Groups, New_Grouping, f_Cr_mcg_per_l, f_Cr_mcg_per_l_qa, p_Cr_mcg_per_l, p_Cr_mcg_per_l_qa) %>%
  filter(!(is.na(New_Grouping)))

# Filter NA values in the grouping variable, convert to long format and handle low/zero values
Cr_long <- Cr_df %>%
  pivot_longer(
    cols = c(f_Cr_mcg_per_l, p_Cr_mcg_per_l),
    names_to = "Metal_type",
    names_pattern = "^(.)_Cr_mcg_per_l",
    values_to = "Metal_conc"
  ) %>%
  filter(!is.na(Metal_conc)) %>%
  mutate(Metal_conc_plot = case_when(
...
```{r}
Cr_plot <- ggplot(Cr_long, aes(x = x_group, y = Metal_conc_plot)) +
  geom_boxplot_pattern(
    aes(fill = New_Groups, pattern = Metal_type, pattern_fill = Metal_type),
    outlier.shape = 16,
    outlier.color = "black",
  ...
  labs(
    title = NULL,
    x = NULL,
    y = "Cr",  # or use ylab_Cr <- "Chromium concentration (µg/L)"
    fill = "Metal Type"
  ) +
  theme_minimal() +
  boxplot_theme +
...
ggsave("C:/Users/tevinger/Box/Poulin Lab ETOX/Project Folders/Alaska BITE_HEAT (Taylor Evinger)/Evinger Manuscripts/Ferrum Manuscript/Figures/02_Global Analysis/Cr_V2 no labels.png", Cr_plot, width = 2.75, height = 3.25, dpi = 300)
```
...
library(plotly)
library(dplyr)
# Make sure data > 0 for log axis
#df <- Al_long %>% filter(Metal_conc_plot > 0)

fig <- plot_ly()

# filtered (f)
fig <- fig %>%
  add_boxplot(data = filter(df, Metal_type == "f"),
              line = list(color = "black", width = 1.5),
              x = ~x_group, y = ~Metal_conc_plot,
              color = ~New_Groups,
              name = ~paste(New_Groups, "(f)"),
              boxmean = TRUE,
              line = list(width = 1),
              opacity = 0.9)
...
# particulate (p)
fig <- fig %>%
  add_boxplot(data = filter(df, Metal_type == "p"),
              line = list(color = "black", width = 1.5),
              x = ~x_group, y = ~Metal_conc_plot,
              color = ~New_Groups,
              name = ~paste(New_Groups, "(p)"),
              boxmean = TRUE,
              line = list(width = 2, dash = "dash"),
              opacity = 0.6,
              showlegend = FALSE)   # avoid duplicate legends

# Layout options to mimic ggplot
fig <- fig %>%
  layout(
    yaxis = list(
      type = "log",
      title = ylab_Al,
      range = log10(c(0.07, 5e6)),
      tickvals = c(0.1, 1, 10, 100, 1000, 10000, 100000, 1e6, 1e7),
      ticktext = c("0.1","1","10","100","1,000","10,000","100,000","1,000,000","10,000,000")
    ),
    xaxis = list(title = ""),
...
fig <- fig %>%
  layout(
    title = list(
      text = "Aluminum Concentrations Across Groups",
      x = 0.5,                  # center the title
      xanchor = "center",
      font = list(size = 18)
    ),
    xaxis = list(title = "Group"),
    yaxis = list(title = "Concentration (µg/L)", type = "log")
  )
