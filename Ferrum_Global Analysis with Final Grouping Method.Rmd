...
Zn_long <- Zn_df %>%
  ...
  filter(!is.na(Metal_conc)) %>% # filtering out here means only the f or p value that is NA is excluded, not both measurements for the same sample
  mutate(Metal_conc_plot = Metal_conc + + ((sqrt(2)/2)*0.1))
...
#ggsave("C:/Users/tevinger/Box/Poulin Lab ETOX/Project Folders/Alaska BITE_HEAT (Taylor Evinger)/Evinger Manuscripts/Ferrum Manuscript/Figures/02_Global Analysis/Zn_V1 labels.png", Zn_plot, width = 2.75, height = 3.25, dpi = 300)
```
