...
discharge_data <- discharge_data_mod %>%
  ...
  mutate(Onset_timing = case_when(
    Year %in% c("2015", "2016", "2017", "2018") ~ "pre",
    Year %in% c("2019", "2020", "2021", "2022", "2023", "2024") ~ "post",
    TRUE ~ NA_character_
  ))
...
```{r}
cumulative_discharge_by_year <- cumulative_discharge_by_year %>%
  arrange(station_nm, Year)

q.p_2 <- ggplot(cumulative_discharge_by_year, 
                aes(x = as.factor(Year), y = total_discharge_cfs, color = station_nm)) +
  geom_point() +
  geom_line(aes(group = 1)) +   # ✅ THIS restores the line
  facet_wrap(station_nm~., scales = "free", ncol = 2) +
...
ggsave("C:/Users/tevinger/Downloads/Total Discharge.png", q.p_2, width = 11, height = 7)
...
discharge_bps <- ggplot(yearly_discharge, 
                aes(x = Onset_timing, y = average_discharge_cfs, color = station_nm)) +
  geom_boxplot() +
  facet_wrap(station_nm~., scales = "free", ncol = 2) +
  labs(x = NULL, y = "Discharge (cfs)") +
  theme_minimal() +
...
ggsave("C:/Users/tevinger/Downloads/Onset timing Discharge.png", discharge_bps, width = 11, height = 7)
```
...
cumulative_discharge_DahlCreek <- cumulative_discharge_by_year_stats %>%
  filter(station_nm == "DAHL C NR KOBUK AK")

cumulative_discharge_KobukRiver <- cumulative_discharge_by_year_stats %>%
  filter(station_nm == "KOBUK R NR KIANA AK")

cumulative_discharge_SlateCreek <- cumulative_discharge_by_year_stats %>%
  filter(station_nm == "SLATE C AT COLDFOOT AK")

cumulative_discharge_WulikRiver <- cumulative_discharge_by_year_stats %>%
...
columns_to_model <- c("total_discharge_cfs")
...
for (col in columns_to_model) {
  ...
  model <- lm(cumulative_discharge_by_year_stats[[col]] ~ cumulative_discharge_by_year_stats$Temporal_status, data = cumulative_discharge_by_year_stats)
...
##Emmeans
```{r}
Dahl_Discharge_data_model <- lm(total_discharge_cfs ~ Temporal_status, data = cumulative_discharge_DahlCreek)
Dahl_Discharge_data_emm <- emmeans(Dahl_Discharge_data_model, ~ Temporal_status)
Dahl_Discharge_data_contrast <- contrast(Dahl_Discharge_data_emm, method = "pairwise")
print(Dahl_Discharge_data_contrast)

Kobuk_Discharge_data_model <- lm(total_discharge_cfs ~ Temporal_status, data = cumulative_discharge_KobukRiver)
Kobuk_Discharge_data_emm <- emmeans(Kobuk_Discharge_data_model, ~ Temporal_status)
Kobuk_Discharge_data_contrast <- contrast(Kobuk_Discharge_data_emm, method = "pairwise")
print(Kobuk_Discharge_data_contrast)

Slate_Discharge_data_model <- lm(total_discharge_cfs ~ Temporal_status, data = cumulative_discharge_SlateCreek)
Slate_Discharge_data_emm <- emmeans(Slate_Discharge_data_model, ~ Temporal_status)
Slate_Discharge_data_contrast <- contrast(Slate_Discharge_data_emm, method = "pairwise")
print(Slate_Discharge_data_contrast)

Wulik_Discharge_data_model <- lm(total_discharge_cfs ~ Temporal_status, data = cumulative_discharge_WulikRiver)
Wulik_Discharge_data_emm <- emmeans(Wulik_Discharge_data_model, ~ Temporal_status)
Wulik_Discharge_data_contrast <- contrast(Wulik_Discharge_data_emm, method = "pairwise")
...
nonparametric_columns <- c("total_discharge_cfs")
...
for (col in nonparametric_columns) {
  ...
  df_filtered <- cumulative_discharge_by_year_stats %>%
    filter(!is.na(.data[[col]]))
  
  # Perform Kruskal-Wallis test
  test_result <- tryCatch(
    {
      kruskal_test(as.formula(paste(col, "~ Temporal_status")), data = df_filtered)
    },
    error = function(e) {
...
#write_xlsx(kruskal_results_df, "C:/Users/tevinger/Downloads/Kruskal Wallis results_f_Al.xlsx")
```
...
columns_for_dunn <- c("f_Al_mcg_per_l")
...
write_xlsx(dunn_results_df, "C:/Users/tevinger/Downloads/Dunns Test results_updated f_Al.xlsx")

write_xlsx(cld_results_df, "C:/Users/tevinger/Downloads/CLD results_updated f_Al.xlsx")
```
...
RD_Sulfate_temporal_bp <- RD_sulfate %>%
  ...
  #scale_y_log10() +
  # scale_y_continuous(breaks = c(0,50,100,150,200,250,300,350,400,450,500,550,600,650,700,750,800,850,900,950,1000,1050,1100), labels = c(0,"","","",200,"","","",400,"","","",600,"","","",800,"","","",1000,"","")) +
  scale_y_continuous(breaks = c(0,100,200,300,400,500,600,700,800,900,1000,1100), labels = c(0,"",200,"",400,"",600,"",800,"",1000,"")) +
...
ggsave("C:/Users/tevinger/Box/Poulin Lab ETOX/Project Folders/Alaska BITE_HEAT (Taylor Evinger)/Evinger Manuscripts/Ferrum Manuscript/Figures/Red Dog Temporal/SO4_2015 to 2024.png", RD_Sulfate_temporal_bp, width = 3.25, height = 4, dpi = 300, bg = "transparent")
```
...
```{r}
RD_Alkalinity_temporal_bp <- RD_alkalinity_ALL %>%
  filter(sample_collection_season %in% c("early", "late")) %>%
  ggplot(aes(x = as.factor(sample_collection_year), y = report_result_value, fill = sample_collection_season)) +
...
ggsave("C:/Users/tevinger/Box/Poulin Lab ETOX/Project Folders/Alaska BITE_HEAT (Taylor Evinger)/Evinger Manuscripts/Ferrum Manuscript/Figures/Red Dog Temporal/Alkalinity with all years.png", RD_Alkalinity_temporal_bp, width = 7, height = 5, dpi = 300, bg = "transparent")
```
...
RD_Zinc_temporal_bp <- RD_zinc %>%
  ...
  #coord_cartesian(ylim = c(5, 110000)) +
  # Add counts as text
  geom_vline(xintercept = 4.5, linetype = "dashed", color = "black", size = 0.8, alpha = 0.7) +
  #geom_vline(xintercept = 21.5, linetype = "dashed", color = "black", size = 0.8, alpha = 0.7) +
  labs(
...
ggsave("C:/Users/tevinger/Box/Poulin Lab ETOX/Project Folders/Alaska BITE_HEAT (Taylor Evinger)/Evinger Manuscripts/Ferrum Manuscript/Figures/Red Dog Temporal/Zinc_2015 to 2024.png", RD_Zinc_temporal_bp, width = 3.25, height = 4, dpi = 300, bg = "transparent")
```
...
ggsave("C:/Users/tevinger/Box/Poulin Lab ETOX/Project Folders/Alaska BITE_HEAT (Taylor Evinger)/Evinger Manuscripts/Ferrum Manuscript/Figures/Red Dog Temporal/Red Dog SO4 to Cl for all years_log.png", RD_SO4_Cl_temporal_bp, width = 7, height = 5, dpi = 300, bg = "transparent")
```
...
```{r}
RD_early_temporal_stats <- RD_early_temporal_stats %>%
 mutate(
    unique_ID = paste(sample_date, analysis_date, sep = "_")) %>%
  relocate(unique_ID, .after = analysis_date)

dups <- RD_early_temporal_stats_wide %>%
  #filter(unique_ID == "2024-06-03_2024-06-03 11:57:00")
...
RD_early_temporal_stats_no_dupes <- RD_early_temporal_stats %>%
  distinct(unique_ID, chemical_name, .keep_all = TRUE)

RD_early_temporal_stats_wide <- RD_early_temporal_stats_no_dupes %>%
  dplyr::select(-c(analysis_date, unique_ID)) 
  pivot_wider(
    id_cols   = c(1:7),   # keep columns 1–7 as identifiers
    names_from  = 8,   # column 8 becomes the new headers
    values_from = 9    # column 9 fills the values
  )
...
write_xlsx(RD_temporal_stats_summary, "C:/Users/tevinger/Downloads/Red Dog Station 9 summary stats.xlsx")
```

Early summary stats
```{r}
RD_temporal_stats_summary_early <- RD_early_temporal_stats %>%
  filter(temporal_status_season %in% c("pre_early", "post_early")) %>%   # keep only the two groups
...
view(RD_temporal_stats_summary_early)

write_xlsx(RD_temporal_stats_summary_early, "C:/Users/tevinger/Downloads/Red Dog Station 9 EARLY SEASON summary stats.xlsx")
```

Late summary stats
```{r}
RD_temporal_stats_summary_late <- RD_late_temporal_stats %>%
  filter(temporal_status_season %in% c("pre_late", "post_late")) %>%   # keep only the two groups
  group_by(chemical_name, temporal_status_season) %>%
  summarise(
    n        = n(),                     # number of values
    mean     = mean(report_result_value, na.rm = TRUE),
...
view(RD_temporal_stats_summary_late)

write_xlsx(RD_temporal_stats_summary_late, "C:/Users/tevinger/Downloads/Red Dog Station 9 LATE SEASON summary stats.xlsx")
```
...
write_xlsx(early_shapiro_results_long, "C:/Users/tevinger/Downloads/early_shapiro_results_long.xlsx")
...
ggsave("C:/Users/tevinger/Box/Poulin Lab ETOX/Project Folders/Alaska BITE_HEAT (Taylor Evinger)/Evinger Manuscripts/Ferrum Manuscript/Figures/04_Agashashok Temporal Analysis/SO4 seasonal temporal bp_V3_smaller.png", Sulfate_temporal_bp, width = 3.25, height = 4, dpi = 300, bg = "transparent")
```
...
## Chloride
```{r}
Chloride_temporal_bp <- temporal_filled %>%
  filter(sample_collection_season %in% c("early", "late")) %>%
...
ggsave("C:/Users/tevinger/Box/Poulin Lab ETOX/Project Folders/Alaska BITE_HEAT (Taylor Evinger)/Evinger Manuscripts/Ferrum Manuscript/Figures/04_Agashashok Temporal Analysis/Aggie SO4 to Cl.png", Aggie_SO4_Cl_temporal_bp, width = 5, height = 5, dpi = 300, bg = "transparent")
```
...
temporal_stats <- Agashashok_for_plots %>%
  filter(sample_collection_season %in% c("early", "late")) %>%
   mutate(
    Temporal_status = ifelse(sample_collection_year %in% c(2015, 2016, 2017, 2018), 'pre',
                      ifelse(sample_collection_year %in% c(2019, 2022, 2023, 2024), 'post', NA))
  ) %>%
  mutate(
    temporal_status_season = paste(Temporal_status, sample_collection_season, sep = "_")
  ) %>%
  relocate(temporal_status_season, .after = SamplingEventID) %>%
  mutate(Temporal_status = factor(Temporal_status, levels = c("pre", "post")))
...
## HCO3 is normally distributed --> t-test
```
...
# Variables that FAILED Shapiro (NON-NORMAL)
non_normal_vars <- c("pH", "Alk_mgCaCO3_per_l", "DIC_mgC_per_l", "DOC_mgC_per_l", "f_Cl_mg_per_l", 
  "f_SO4_mg_per_l", "f_Ca_mg_l", "f_Mg_mg_l", "f_Na_mg_l", "f_K_mg_l")
...
# Variables that FAILED Shapiro (NON-NORMAL)
non_normal_vars <- c("DOC_mgC_per_l", "HCO3_mg_L", "f_Cl_mg_per_l", 
