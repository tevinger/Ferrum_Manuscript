...
discharge_bps <- discharge_data %>%
  filter(!is.na(onset_season)) %>%
  mutate(
    pre_flag = ifelse(grepl("^pre", onset_season), "pre", "post")
  ) %>%
  ggplot(aes(
    x = onset_season,
    y = discharge_cms,
  ...
  scale_y_log10() +
  scale_fill_manual(values = fill_values) +
  scale_pattern_manual(values = c(pre = "circle", post = "none")) +
  labs(x = NULL, y = NULL) +
  theme_minimal() +
  theme(
    axis.text.y = element_text(size = 12, color = "black", face = "bold"),
...
ggsave("C:/Users/tevinger/Downloads/Raw Discharge by Onset timing and season_V2.png", discharge_bps, width = 10, height = 5)
```
...
```{r}
Snow_Kobuk <- read_excel("C:/Users/tevinger/Box/Poulin Lab ETOX/Project Folders/Alaska BITE_HEAT (Taylor Evinger)/Evinger Manuscripts/Ferrum Manuscript/Re_ [EXTERNAL] Climate Data for Ferrum Paper/Snow Kobuk Valley.xlsx",
                                    sheet = "Sheet1")

Temp_Kobuk <- read_excel("C:/Users/tevinger/Box/Poulin Lab ETOX/Project Folders/Alaska BITE_HEAT (Taylor Evinger)/Evinger Manuscripts/Ferrum Manuscript/Re_ [EXTERNAL] Climate Data for Ferrum Paper/T2M Kobuk Valley.xlsx",
                                    sheet = "Sheet1")
```

```{r}
Snow_Kobuk <- Snow_Kobuk %>%
  dplyr::rename(
    Snowfall_cm = Value,
    Snowfall_rank = ValueRank
  ) 

Temp_Kobuk <- Temp_Kobuk %>%
  dplyr::rename(
    Temp_C = Value,
    Temp_rank = ValueRank
  )
...
```{r}
combined_df <- Snow_Kobuk %>%
  left_join(Temp_Kobuk, by = "Year")

combined_df <- combined_df %>%
  mutate(
    year_chr = as.character(Year),
    year_group = case_when(
      str_starts(year_chr, "194") | str_starts(year_chr, "195") | str_starts(year_chr, "196") ~ "group1",
      str_starts(year_chr, "197") | str_starts(year_chr, "198") | str_starts(year_chr, "199") ~ "group2",
      str_starts(year_chr, "200") ~ "group3",
      str_starts(year_chr, "201") ~ "group4",
      str_starts(year_chr, "202") ~ "group5",
      TRUE ~ NA_character_
    )
  ) %>%
  dplyr::select(-year_chr)
```

##plot
```{r}
climate_plot_SI <- ggplot(combined_df, aes(x = Temp_C, y = Snowfall_cm, color = year_group)) +
  ...
  coord_cartesian(ylim = c(45,175), xlim = c(-10,10)) +
  scale_y_continuous(
  ...
  # ) +
  geom_text(
    data = combined_df %>% dplyr::filter(year_group == "group4"),
    aes(label = Year),
    vjust = -0.9,
    size = 3,
    show.legend = FALSE
...
ggsave("C:/Users/tevinger/Downloads/climate data no legend.png", climate_plot_SI, width = 6, height = 6, dpi = 300, bg = "transparent")
```
...
ggsave("C:/Users/tevinger/Box/Poulin Lab ETOX/Project Folders/Alaska BITE_HEAT (Taylor Evinger)/Evinger Manuscripts/Ferrum Manuscript/Figures/Red Dog Temporal/1_26_26_new Alk with labels.png", RD_Alkalinity_temporal_bp, width = 4, height = 4.35, dpi = 300, bg = "transparent")
```
