...
RD_Zinc_temporal_bp <- RD_zinc %>%
  ...
  scale_y_continuous(breaks = c(0,500,1000,1500,2000,2500,3000,3500,4000,4500,5000), labels = c(0,"",1000,"",2000,"",3000,"",4000,"",5000)) +
  geom_vline(xintercept = 9.5, linetype = "dashed", color = "black", size = 0.8, alpha = 0.7) +
  labs(
    title = NULL,
...
ggsave("C:/Users/tevinger/Box/Poulin Lab ETOX/Project Folders/Alaska BITE_HEAT (Taylor Evinger)/Evinger Manuscripts/Ferrum Manuscript/Figures/Red Dog Temporal/Final_Zn no ticks.png", RD_Zinc_temporal_bp, width = 4, height = 4.35, dpi = 300, bg = "transparent")
```
...
Sulfate_temporal_bp <- temporal_filled_2 %>%
  filter(sample_collection_season %in% c("early", "late")) %>%
  ggplot(aes(x = as.factor(year_season), y = f_SO4_mg_per_l, fill = sample_collection_season)) +
  #ggplot(aes(x = as.factor(sample_collection_year), y = f_SO4_mg_per_l, fill = sample_collection_season)) +
  geom_boxplot(
    position = position_dodge2(width = 0.75), width = 0.55
    ) +
  scale_x_discrete(expand = expansion(add = 0.6)) +
  scale_y_continuous(breaks = c(0,25,50,75,100,125,150,175,200,225,250,275), labels = c(0, "", 50, "", 100, "", 150, "", 200, "", 250, "")) +
  #scale_x_continuous(expand = expansion(mult = c(0.035, 0.025))) +
  scale_fill_manual(
    values = c("early" = "#A1E3F9", "late" = "#0a7ea4")
  ) +
