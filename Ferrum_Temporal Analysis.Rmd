...
ggsave("C:/Users/tevinger/Box/Poulin Lab ETOX/Project Folders/Alaska BITE_HEAT (Taylor Evinger)/Evinger Manuscripts/Ferrum Manuscript/Figures/04_Agashashok Temporal Analysis/SO4 seasonal temporal bp_V2.png", Sulfate_temporal_bp, width = 5, height = 5, dpi = 300, bg = "transparent")
```

## Alkalinity
```{r}
Alkalinity_temporal_bp <- temporal_filled %>%
  filter(sample_collection_season %in% c("early", "late")) %>%
...
pre_temporal_stats <- temporal_stats %>%
  filter(Temporal_status == "pre") %>%
  mutate(sample_collection_season = factor(sample_collection_season, levels = c("early", "late")))

post_temporal_stats <- temporal_stats %>%
  filter(Temporal_status == "post") %>%
  mutate(sample_collection_season = factor(sample_collection_season, levels = c("early", "late")))
```
...
for (col in columns_to_model) {
  ...
  model <- lm(pre_temporal_stats[[col]] ~ pre_temporal_stats$sample_collection_season, data = pre_temporal_stats)
...
pre_shapiro_results <- bind_rows(model_results)

# Print results
view(pre_shapiro_results)

## Alkalinity and Magnesium are normally distributed and need to be compared with an ANOVA
```
...
nonparametric_columns <- c("pH", "DOC_mgC_per_l", "f_Cl_mg_per_l", 
...
for (col in nonparametric_columns) {
  ...
  df_filtered <- pre_temporal_stats %>%
    filter(!is.na(.data[[col]]))
  
  # Perform Kruskal-Wallis test comparing sample_collection_season values
  test_result <- tryCatch(
    {
      result <- kruskal_test(as.formula(paste(col, "~ sample_collection_season")), data = df_filtered)
...
pre_kruskal_results_df <- bind_rows(kruskal_results_list)


# Print the results
view(pre_kruskal_results_df)

## no difference in groups for pH and DOC

write_xlsx(pre_kruskal_results_df, "C:/Users/tevinger/Box/Poulin Lab ETOX/Project Folders/Alaska BITE_HEAT (Taylor Evinger)/Evinger Manuscripts/Ferrum Manuscript/Results and Discussion/Agashashok Temporal Analysis/pre_kruskal_results.xlsx")
```
...
columns_for_dunn <- c("f_Cl_mg_per_l", "f_SO4_mg_per_l", "f_Ca_mg_l", "f_Na_mg_l", "f_K_mg_l")
...
for (col in columns_for_dunn) {
  ...
  test_result <- tryCatch(
    {
      df_filtered %>%
        dunn_test(as.formula(paste(col, "~ sample_collection_season")), p.adjust.method = "bonferroni")
    },
    error = function(e) {
      # Return a default result if test fails
      data.frame(
...
pre_dunn_results_df <- bind_rows(dunn_results_list)

# Combine all CLD results into a single dataframe
pre_cld_results_df <- bind_rows(cld_results_list)

# Print the results
view(pre_dunn_results_df)
view(pre_cld_results_df)

write_xlsx(pre_dunn_results_df, "C:/Users/tevinger/Box/Poulin Lab ETOX/Project Folders/Alaska BITE_HEAT (Taylor Evinger)/Evinger Manuscripts/Ferrum Manuscript/Results and Discussion/Agashashok Temporal Analysis/pre_dunn_results.xlsx")

write_xlsx(pre_cld_results_df, "C:/Users/tevinger/Box/Poulin Lab ETOX/Project Folders/Alaska BITE_HEAT (Taylor Evinger)/Evinger Manuscripts/Ferrum Manuscript/Results and Discussion/Agashashok Temporal Analysis/pre_cld_results.xlsx")
```
...
for (col in columns_to_model) {
  ...
  # Fit the model (example: linear model with sample_collection_season as predictor)
  model <- lm(post_temporal_stats[[col]] ~ post_temporal_stats$sample_collection_season, data = post_temporal_stats)
...
# Combine all results into a single data frame
post_shapiro_results <- bind_rows(model_results)

# Print results
view(post_shapiro_results)
