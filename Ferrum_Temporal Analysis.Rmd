...
## ~ 1 in the model means there is no predictor variable being fit to the model

shapiro_results <- discharge_data_for_stats %>%
  group_by(station_nm) %>%
  summarise(
    n = sum(!is.na(discharge_cms)),

    test_result = list(
      if (n >= 3) shapiro.test(resid(lm(discharge_cms ~ 1,
                                       data = pick(discharge_cms),
                                       na.action = na.omit)))
      else NULL
    ),
...
wilcox_by_station <- discharge_data_for_stats %>%
  filter(!is.na(discharge_cms), Onset_timing %in% c("pre", "post")) %>%
  group_by(station_nm) %>%
  summarise(
    ...
    test_result = list(
      if (n_pre >= 1 && n_post >= 1) {
        wilcox.test(discharge_cms ~ Onset_timing, data = pick(discharge_cms, Onset_timing),
                    exact = FALSE)
      } else {
        NULL
      }
    ...
    Interpretation = if (!is.null(test_result[[1]]) && test_result[[1]]$p.value > 0.05) {
      "No evidence of a difference between pre and post (fail to reject H0)."
    } else if (!is.null(test_result[[1]])) {
      "Evidence of a difference between pre and post (reject H0)."
    } else {
      "Test could not be performed (need at least one pre and one post value)."
    },
...
##Emmeans
```{r}
Dahl_Discharge_data_model <- lm(total_discharge_cms ~ onset_timing, data = cumulative_discharge_DahlCreek)
...
Kobuk_Discharge_data_model <- lm(total_discharge_cms ~ onset_timing, data = cumulative_discharge_KobukRiver)
...
Slate_Discharge_data_model <- lm(total_discharge_cms ~ onset_timing, data = cumulative_discharge_SlateCreek)
...
Wulik_Discharge_data_model <- lm(total_discharge_cms ~ onset_timing, data = cumulative_discharge_WulikRiver)
...
nonparametric_columns <- c("total_discharge_cms")
...
write_xlsx(early_shapiro_results_long, "C:/Users/tevinger/Downloads/early_shapiro_results_long_12 30 2025.xlsx")
...
write_xlsx(wilcox_results, "C:/Users/tevinger/Downloads/early wilcox_results_long_12 30 2025.xlsx")
```
...
write_xlsx(late_shapiro_results_long, "C:/Users/tevinger/Downloads/LATE shapiro_results_long_12 30 2025.xlsx")
...
write_xlsx(wilcox, "C:/Users/tevinger/Downloads/LATE wilcox_results_long_12 30 2025.xlsx")
```
...
write_xlsx(t_test_results, "C:/Users/tevinger/Downloads/LATE t test_results_long_12 30 2025.xlsx")
```
...
write_xlsx(early_shapiro_results, "C:/Users/tevinger/Downloads/EARLY shapiro_results_long.xlsx")
...
write_xlsx(wilcox_results_wide, "C:/Users/tevinger/Downloads/EARLY Wilcox_results_long.xlsx")
```
...
write_xlsx(t_test_results, "C:/Users/tevinger/Downloads/EARLY t test_results_long.xlsx")
```
...
write_xlsx(late_shapiro_results, "C:/Users/tevinger/Downloads/LATE Shapiro_results_long.xlsx")
```
...
write_xlsx(wilcox_results_wide, "C:/Users/tevinger/Downloads/LATE Wilcox_results_long.xlsx")
```
...
write_xlsx(t_test_results, "C:/Users/tevinger/Downloads/LATE t test_results_long.xlsx")
```
