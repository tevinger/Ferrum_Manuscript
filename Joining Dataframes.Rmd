...
columns_to_check <- c(
  ...
  "p_Al_mcg_per_l", "p_V_mcg_per_l", 
  "p_Cr_mcg_per_l", "p_Mn_mcg_per_l", 
  "p_Fe_mcg_per_l", "p_Co_mcg_per_l", 
  "p_Ni_mcg_per_l", "p_Cu_mcg_per_l", 
  "p_Zn_mcg_per_l", "p_As_mcg_per_l", 
  "p_Se_mcg_per_l", "p_Y_mcg_per_l", 
  "p_Ag_mcg_per_l", "p_Cd_mcg_per_l", 
  "p_Ba_mcg_per_l", "p_La_mcg_per_l", 
  "p_Ce_mcg_per_l", "p_Pr_mcg_per_l", 
  "p_Nd_mcg_per_l", "p_Dy_mcg_per_l", 
  "p_Tl_mcg_per_l", "p_U_mcg_per_l", 
  "p_Pb_mcg_per_l"
)


# count the number of cells in each qa column that is <DL to calculate the percent of total data
DL_counts <- sapply(columns_to_check_qa, function(col) {
  sum(grepl("DL", Alaska_DataRelease_2022_2023_Ferrum_V10[[col]], ignore.case = TRUE), na.rm = TRUE)
})

# print(DL_counts_df)

numeric_counts <- sapply(columns_to_check, function(col) {
  vals <- Alaska_DataRelease_2022_2023_Ferrum_V10[[col]]
...
DL_summary <- data.frame(
  qa_column        = columns_to_check_qa,
  data_column      = columns_to_check,
  DL_count         = DL_counts,
  numeric_cell_count = numeric_counts,
  row.names = NULL
)
...
DL_summary <- DL_summary %>%
  mutate(
    percent_DL = (DL_count / numeric_cell_count) *100
  )
...
write_xlsx(DL_summary, "C:/Users/tevinger/Box/Poulin Lab ETOX/Project Folders/Alaska BITE_HEAT (Taylor Evinger)/Evinger Manuscripts/Ferrum Manuscript/Datasheets/Ferrum Manuscript Datasheets/left-censored data assessment.xlsx")
```
...
```{r}
Alaska_DataRelease_2022_2023_Ferrum_V11 <- read_excel("C:/Users/tevinger/Box/Poulin Lab ETOX/Project Folders/Alaska BITE_HEAT (Taylor Evinger)/Evinger Manuscripts/Ferrum Manuscript/Datasheets/Ferrum Manuscript Datasheets/Alaska_DataRelease_2022_2023_Ferrum_V11_molar_excel.xlsx")
```

```{r}
# remove 0 values in the columns I converted to mol/L

special_cols <- c(41,45,59,63,67,69,75,89,132,134,136,140,142,144,146,158)
even_cols    <- seq(160, 296, by = 2)
cols_target  <- c(special_cols, even_cols)
print(cols_target)

# keep only indices that exist in df
cols_target <- cols_target[cols_target <= ncol(Alaska_DataRelease_2022_2023_Ferrum_V11)]

# --- replace 0 with NA across those columns ---
Alaska_DataRelease_2022_2023_Ferrum_V11_V2 <- Alaska_DataRelease_2022_2023_Ferrum_V11 %>%
  mutate(
    across(
      all_of(names(Alaska_DataRelease_2022_2023_Ferrum_V11)[cols_target]),
      ~ {
        x <- suppressWarnings(as.numeric(.x))  # force numeric
        na_if(x, 0)                            # replace 0 with NA
      }
    )
  )
```
...
```{r}
# Add REE sum back to the df
Alaska_DataRelease_2022_2023_Ferrum_V11_V3 <- Alaska_DataRelease_2022_2023_Ferrum_V11_V2 %>%
  mutate(
    f_REE = ifelse(
      rowSums(is.na(dplyr::select(., 
                           f_Ce_mol_L, 
  ...
  relocate(f_REE, .after = p_Pb_mol_L_qa) %>%
  relocate(p_REE, .after = f_REE_qa)

view(Alaska_DataRelease_2022_2023_Ferrum_V11_V3)

write_xlsx(Alaska_DataRelease_2022_2023_Ferrum_V11_V3, "C:/Users/tevinger/Box/Poulin Lab ETOX/Project Folders/Alaska BITE_HEAT (Taylor Evinger)/Evinger Manuscripts/Ferrum Manuscript/Datasheets/Ferrum Manuscript Datasheets/Alaska_DataRelease_2022_2023_Ferrum_V11_molar.xlsx")
```
...
All_Seep_Data <- Alaska_DataRelease_2022_2023_Ferrum_V10 %>%
  filter(site_classification == "seep")

# add sum of metals
# Identify the columns of interest
cols_to_sum <- names(All_Seep_Data)[161:301] %>%
  # keep those starting with f_
  .[grepl("^f_", .)] %>%
  # drop those ending with _qa
  .[!grepl("_qa$", .)]

# sum of all metals
All_Seep_Data <- All_Seep_Data %>%
  mutate(across(all_of(cols_to_sum), ~ suppressWarnings(as.numeric(.)))) %>%
  mutate(all_sum_of_metals = rowSums(across(all_of(cols_to_sum)), na.rm = TRUE))

# sum of Zn, Cu, Cd, Pb, Co and Ni
Ficklin_cols_to_sum <- names(All_Seep_Data)[161:301] %>%
  # must start with f_
  .[grepl("^f_", .)] %>%
  # must not end with _qa
  .[!grepl("_qa$", .)] %>%
  # must contain one of Zn, Cu, Cd, Pb, Co, Ni
  .[grepl("Zn|Cu|Cd|Pb|Co|Ni", .)]

# Step 2: Convert those to numeric and sum
All_Seep_Data <- All_Seep_Data %>%
  mutate(across(all_of(Ficklin_cols_to_sum), ~ suppressWarnings(as.numeric(.)))) %>%
  mutate(Ficklin_sum_of_metals = rowSums(across(all_of(Ficklin_cols_to_sum)), na.rm = TRUE))

write_xlsx(All_Seep_Data, "C:/Users/tevinger/Box/Poulin Lab ETOX/Project Folders/Alaska BITE_HEAT (Taylor Evinger)/Evinger Manuscripts/Ferrum Manuscript/Datasheets/Ferrum Manuscript Datasheets/All_seep_Data.xlsx")
```
...
All_Seep_Data_molar <- Alaska_DataRelease_2022_2023_Ferrum_V11_molar %>%
  filter(site_classification == "seep")

# add sum of metals
# Identify the columns of interest
cols_to_sum <- names(All_Seep_Data_molar)[161:301] %>%
  # keep those starting with f_
  .[grepl("^f_", .)] %>%
  # drop those ending with _qa
  .[!grepl("_qa$", .)]
...
# sum of all metals
All_Seep_Data_molar <- All_Seep_Data_molar %>%
  mutate(across(all_of(cols_to_sum), ~ suppressWarnings(as.numeric(.)))) %>%
  mutate(all_sum_of_metals = rowSums(across(all_of(cols_to_sum)), na.rm = TRUE))

# sum of Zn, Cu, Cd, Pb, Co and Ni
Ficklin_cols_to_sum <- names(All_Seep_Data_molar)[161:301] %>%
  # must start with f_
  .[grepl("^f_", .)] %>%
  # must not end with _qa
  .[!grepl("_qa$", .)] %>%
...
write_xlsx(All_Seep_Data_molar, "C:/Users/tevinger/Box/Poulin Lab ETOX/Project Folders/Alaska BITE_HEAT (Taylor Evinger)/Evinger Manuscripts/Ferrum Manuscript/Datasheets/Ferrum Manuscript Datasheets/All_seep_Data_molar.xlsx")
```