...
grouped_stats_working_Ferrum_data <- Alaska_DataRelease_2022_2023_Ferrum_V7 %>%
  dplyr::select(ParkID,
                Watershed,
                New_Grouping,
...
view(grouped_stats_working_Ferrum_data)

# filter out seep samples - use for summary stats of all data, grouped by watershed, year, and month
stats_working_Ferrum_data <- Alaska_DataRelease_2022_2023_Ferrum_V7 %>%
  dplyr::select(ParkID,
                Watershed,
                New_Grouping,
                sample_collection_year,
...
### set elements and columns for stats loop
```{r}
#summary stats
library(dplyr)
...
for (i in seq_along(elements)) {
  ...
  summary_stats <- 
    ...
    #dplyr::select(3,9:70) %>% # New Grouping
    filter(!is.na(.data[[column]])) %>%
    group_by(sample_collection_month) %>% #This is where you can change what variable you want to group the data by
    summarise(
    Variable = element,
    Mean = round(mean(.data[[column]], na.rm = TRUE), 2),
    SD = round(sd(.data[[column]], na.rm = TRUE), 2),
...
write_xlsx(summary_stats_results, "C:/Users/tevinger/OneDrive - University of California, Davis/Lab/Alaska Projects/Summary Stats/By Month_seeps excluded.xlsx")
```
