...
for (i in seq_along(elements)) {
  ...
  summary_stats <- 
    ...
    dplyr::select(3,4,19:80) %>% # New Grouping
...
write_xlsx(summary_stats_results, "C:/Users/tevinger/Downloads/Season_seeps excluded.xlsx")
```
...
for (i in seq_along(elements)) {
  ...
  # Perform the summary operation for the current element
  summary <- Grouped_Ferrum_2022_2023_data %>%
    dplyr::select(4,19:82) %>%  # adjust as needed
    filter(!is.na(.data[[column]])) %>%
  ...
  q1_q3 <- Grouped_Ferrum_2022_2023_data %>%
    dplyr::select(4,19:82) %>%  # adjust as needed
    filter(!is.na(.data[[column]])) %>%
...
write_xlsx(summary_df, "C:/Users/tevinger/Downloads/Group summary stats_rstatix.xlsx")
```
...
write_xlsx(summary_df_filtered_wide, "C:/Users/tevinger/Box/Poulin Lab ETOX/Project Folders/Alaska BITE_HEAT (Taylor Evinger)/Evinger Manuscripts/Ferrum Manuscript/Results and Discussion/Ferrum Manuscript Summary Stats/Upstream, seep, downstream_medians.xlsx")
```
...
shapiro_kruskal_results <- shapiro_kruskal_results %>%
  ...
  mutate(
    shap_p_value = round(shap_p_value, 5),
...
write_xlsx(dunn_results_df, "C:/Users/tevinger/Box/Poulin Lab ETOX/Project Folders/Alaska BITE_HEAT (Taylor Evinger)/Evinger Manuscripts/Ferrum Manuscript/Results and Discussion/Group Comparisons/Dunns Test results with CLD.xlsx")
```
