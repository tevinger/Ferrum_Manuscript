...
```{r}
# filter out samples without a New_Group - use for summary stats of the groups
Molar_Grouped_Ferrum_2022_2023_data <- 
  #Alaska_DataRelease_2022_2023_Ferrum_V10 %>%
  Alaska_DataRelease_2022_2023_Ferrum_V11_molar %>%
  dplyr::select(ParkID,
                Watershed,
...
Molar_Grouped_Ferrum_2022_2023_data <- Molar_Grouped_Ferrum_2022_2023_data %>%
  mutate(across(17:104, as.numeric))

view(Molar_Grouped_Ferrum_2022_2023_data)
```

### All data excluding seeps
currently this keeps all 2022 data and all sites that don't have an assigned group. This includes sites from Aggie SF which may be better to have excluded when doing watershed stats?
```{r}
...
f_REE, p_REE
  ...
  mutate(across(17:83, as.numeric))
...
for (i in seq_along(elements)) {
  ...
  summary_stats <- 
    ...
    dplyr::select(2,4,19:86) %>% # New Grouping and Watershed
...
write_xlsx(summary_stats_results, "C:/Users/tevinger/Downloads/Group and Watershed summary stats_updated Dec 7_2025.xlsx")
```
...
write_xlsx(summary_combined, "C:/Users/tevinger/Downloads/Group and watershed summary stats_rstatix_Dec 7_2025.xlsx")
```
...
# Updated Aluminum Stats
## Summary stats
```{r}
elements <- c("f_Al")
columns <- c("f_Al_mcg_per_l")
...
#summary_stats_results <- summary_stats_results %>% arrange(Watershed)

view(summary_stats_results)

write_xlsx(summary_stats_results, "C:/Users/tevinger/Downloads/Summ.Stats.by Group_updated Al.xlsx")
```
...
# List of columns to analyze
columns_to_model <- c("f_Al_mcg_per_l")
...
nonparametric_columns <- c("f_Al_mcg_per_l")
...
DO_name <- c("DO_mg_L", "DO_percent")
DO_column <- c("Diss_oxy_mg_per_l", "Diss_oxy_percent_sat")
```
...
run_kruskal_analysis <- function(data, DO_column, grouping_var = "New_Grouping") {
...
# Usage example:
my_results <- run_kruskal_analysis(Grouped_Ferrum_2022_2023_data, DO_column)
view(my_results)
...
write_xlsx(dunn_results_df_2, "C:/Users/tevinger/Box/Poulin Lab ETOX/Project Folders/Alaska BITE_HEAT (Taylor Evinger)/Evinger Manuscripts/Ferrum Manuscript/Results and Discussion/Group Comparisons/Dunns Test results with CLD_Dissolved Oxygen.xlsx")
```
