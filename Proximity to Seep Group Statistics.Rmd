...
for (col in columns_to_model) {
  ...
  effect_size_result <- tryCatch(
    {
      df_filtered %>%
        kruskal_effsize(as.formula(paste(col, "~ New_Grouping")), ci = TRUE)
    },
    error = function(e) {
      data.frame(
        .y. = col,
  ...
  merged_result <- left_join(
    ...
    effect_size_result %>% dplyr::select(.y., effsize, conf.low, conf.high),
    by = c(".y." = ".y.")
  )
  ...
  # Store the result
  kruskal_results_list[[col]] <- merged_result
}
