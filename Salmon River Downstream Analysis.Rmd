...
```{r}
mod_data <- data.frame(
  Rel_Water_Area_plotting = c(4.7, 6.2, 7.95, 8.05, 8.95, 10.85, 10.90, 10.95, 22.5),
  rel_u_Fe = c(1, 0.2, 0.02, 0.03, 0.02, 0.7, 0.6, 0.5, 0),
  rel_f_Fe = c(8, 0.2, 0.2, 0.3, 0.1, 0.1, 0.09, 0.05, 0),
  rel_p_Fe = c(1, 0.2, 0.02, 0.02, 0.01, 1.15, 1.05, 1, 0)
) %>%
...
rel_data_to_plot_interp_stine <- rel_data_mod %>%
  mutate(across(
    .cols = where(is.numeric) & !matches("Rel_Water_Area_plotting"),
