...
```{r}
mod_data <- data.frame(
  Rel_Water_Area_plotting = c(4.7, 5.6, 6.2, 7.95, 8.05, 8.95, 10.85, 10.90, 10.95, 14),
  rel_u_Fe = c(1, 0.2, 0.02, 0.03, 0.02, 0.7, 0.6, 0.5, 0, 0),
  rel_f_Fe = c(8, 15, 0.2, 0.3, 0.1, 0.1, 0.09, 0.05, 0, 0),
  rel_p_Fe = c(1, 0.2, 0.02, 0.02, 0.01, 1.15, 1.05, 1, 0, 0)
) %>%
  mutate(across(everything(), as.numeric))
...
rel_data_mod2 <- rel_data_mod %>%
  mutate(rel_f_Fe = case_when(
    Rel_Water_Area_plotting == 4.75  ~ 30,
...
SalmonR_Fe_area_no_tribs <- ggplot() +
  ...
  scale_x_continuous(breaks = DA_x_breaks, labels = DA_x_labels, expand = expansion(mult = c(0.03, 0.02))) +
...
ggsave("C:/Users/tevinger/Box/Poulin Lab ETOX/Project Folders/Alaska BITE_HEAT (Taylor Evinger)/Evinger Manuscripts/Ferrum Manuscript/Figures/03_Downstream Analysis/Fe area plus heat plot_V2.png", SalmonR_Fe_area_combo, width = 5, height = 2.7, dpi = 300)
```
