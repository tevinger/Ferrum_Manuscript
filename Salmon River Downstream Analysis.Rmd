...
rel_data_to_plot_interp <- rel_data_to_plot %>%
  mutate(across(
...
rel_data_to_plot_interp_spline <- rel_data_to_plot %>%
  mutate(across(
    .cols = where(is.numeric) & !matches("Rel_Water_Area_plotting"),
...
rel_data_to_plot_interp_stine <- rel_data_to_plot %>%
  mutate(across(
    .cols = where(is.numeric) & !matches("Rel_Water_Area_plotting"),
...
filled_series <- rel_data_to_plot %>%
  mutate(across(
...
SalmonR_Fe_area <- ggplot() +
  geom_tile(data = trib_f_Fe_area,
            aes(x = Rel_Water_Area_plotting, y = 100, fill = Metal_conc),
            color = "white",
            height = 100,  # controls thickness of tile band
            width = 0.15) +  # controls horizontal width
  scale_fill_gradient(low = "lightblue", high = "darkred", name = "Heatmap\nFe conc") +
...
```{r}
f_Fe_heat_kalman <- ggplot(filled_series, aes(x = Rel_Water_Area_plotting, y = 1, fill = rel_f_Fe)) +
  geom_tile(color = NA, width = 0.05) +
  geom_vline(
  ...
  scale_fill_gradientn(
    ...
    values = scales::rescale(c(0, 1, max(filled_series$rel_f_Fe, na.rm = TRUE))),
    limits = c(0, max(filled_series$rel_f_Fe, na.rm = TRUE)),
    oob = scales::squish
  ) +
  labs(
    x = "Relative Water Area",
