...
write_xlsx(SalmonR_2023_river_data, "C:/Users/tevinger/Box/Poulin Lab ETOX/Project Folders/Alaska BITE_HEAT (Taylor Evinger)/Evinger Manuscripts/Ferrum Manuscript/Datasheets/Ferrum Manuscript Datasheets/Salmon River July 2023 Data.xlsx")
```
...
SalmonR_2023_river_data <- read_excel("C:/Users/tevinger/Box/Poulin Lab ETOX/Project Folders/Alaska BITE_HEAT (Taylor Evinger)/Evinger Manuscripts/Ferrum Manuscript/Datasheets/Ferrum Manuscript Datasheets/Salmon River July 2023 Data_updated.xlsx")
...
SalmonR_filtered_MS <- SalmonR_2023_river_data %>%
  ...
  mutate(New_Grouping = ifelse(is.na(New_Grouping), "normal", New_Grouping))

SalmonR_filtered_Trib <- SalmonR_2023_river_data %>%
  filter(Hyd_Classification == "Tributary") %>%
  mutate(New_Grouping = ifelse(New_Grouping == "Downstream MS", "Impaired Tributary", New_Grouping))

Salmon_impaired_tribs <- SalmonR_filtered_Trib %>%
  dplyr::filter(Site_Classification == "Impaired")

Salmon_unimpaired_tribs <- SalmonR_filtered_Trib %>%
...
u_Zn_mcg_per_l) %>%
  ...
  filter(Distance_km %in% c(0, 103.23)) %>%
  dplyr::select(-c(Distance_km)) %>%
    pivot_longer(
    cols = -c(SiteID),
    names_to = "variable",
    values_to = "value"
  ...
  filter(!is.na(SalmonR_MS2)) %>%
  mutate(downstream_diff_conc = SalmonR_MS6 - SalmonR_MS2) %>%
  mutate(downstream_diff = (SalmonR_MS6/SalmonR_MS2)) %>%
  mutate(downstream_diff_percent = (SalmonR_MS6/SalmonR_MS2) * 100)

view(Salmon_downstream_difference)

#write_xlsx(Salmon_downstream_difference, "C:/Users/tevinger/Box/Poulin Lab ETOX/Project Folders/Alaska BITE_HEAT (Taylor Evinger)/Evinger Manuscripts/Ferrum Manuscript/Results and Discussion/Salmon River Downstream Differences.xlsx")
```
...
## pH
```{r}
DA_x_breaks <- c(1,2.5,5,7.5,10,12.5,15,17.5,20,22.5)
...
```{r}
SalmonR_Fe_area <- ggplot() +
  # filtered tributary heat map
  geom_tile(data = rel_data_to_plot_interp,
            aes(x = Rel_Water_Area_plotting, y = 50000, fill = rel_f_Fe),
            color = NA,
            height = 0.3,  # controls thickness of tile band
            width = 0.05) +  # controls horizontal width
   
  # particulate tributary heat map
  geom_tile(data = rel_data_to_plot_interp,
            aes(x = Rel_Water_Area_plotting, y = 10000, fill = rel_p_Fe),
            color = NA,
            height = 0.3,  # controls thickness of tile band
            width = 0.05) +  # controls horizontal width
  ...
  geom_line(data = MS_u_Fe_area, aes(x = Rel_Water_Area_plotting, y = Metal_conc), color = "#1d6e66", linewidth = 0.5) +
  
  geom_area(data = MS_f_Fe_area, 
            aes(x = Rel_Water_Area_plotting, y = Metal_conc),
            fill = "#c9f1e4") + # light blue/green for filtered
            #fill = "#28968b") + # darker green for filtered paired with grey for particulate
            #fill = "#caf1ed",
  ...
  coord_cartesian(x = c(1,22.5), y = c(1,100000)) +
  scale_x_continuous(breaks = DA_x_breaks, labels = DA_x_labels, expand = expansion(mult = c(0.02, 0.01))) +
  scale_y_log10(breaks = c(0.01,0.1,1,10,100,1000,10000), labels = scales::comma, expand = expansion(mult = c(0.02, 0.02))) +
...
SalmonR_Zn_area <- ggplot() +
  ...
  geom_line(data = MS_u_Zn_area, aes(x = Rel_Water_Area_plotting, y = Metal_conc), color = "#1d6e66", linewidth = 0.5) +
  ...
  #coord_cartesian(x = c(1,22.5), y = c(1,100)) +
  #scale_x_continuous(breaks = DA_x_breaks, labels = DA_x_labels, expand = expansion(mult = c(0.02, 0.01))) +
  #scale_y_log10(breaks = c(0.01,0.1,1,10,100,1000,10000), labels = scales::comma, expand = expansion(mult = c(0.02, 0.02))) +
  #annotation_logticks(sides = "l") +
  labs(
